 ...
 */
function parseStyleWithInheritance(el, inherited) {
    ...
    return {
        ...
        strokeLinecap: el.hasAttribute('stroke-linecap')
            ? el.getAttribute('stroke-linecap') || 'butt'
...
function parseTransformAnimations(el) {
    const anims = [];
    for (let i = 0; i < el.children.length; i++) {
        ...
        if (valuesAttr) {
            valStrs = valuesAttr.split(';').map((s) => s.trim()).filter(Boolean);
        }
        else {
            valStrs = [
        ...
        const values = valStrs.map((s) => s.split(/[\s,]+/).map(Number));
        ...
        if (ktAttr) {
            keyTimes = ktAttr.split(';').map((s) => parseFloat(s.trim()));
        }
        let keySplines = null;
        const ksAttr = child.getAttribute('keySplines');
        if (ksAttr) {
            keySplines = ksAttr.split(';').map((s) => s.trim().split(/[\s,]+/).map(Number));
        }
        anims.push({ transformType, values, dur, begin, additive, keyTimes, calcMode, keySplines });
    }
    return anims;
}
function parsePathAnimations(el) {
    const anims = [];
    for (let i = 0; i < el.children.length; i++) {
        const child = el.children[i];
        if (child.tagName !== 'animate')
            continue;
        if (child.getAttribute('attributeName') !== 'd')
            continue;
        const valuesAttr = child.getAttribute('values');
        if (!valuesAttr)
            continue;
        const keyframes = valuesAttr.split(';').map((s) => s.trim()).filter(Boolean);
        if (keyframes.length < 2)
            continue;
        const numRegex = /-?[\d.]+/g;
        const keyframeNums = keyframes.map((kf) => (kf.match(numRegex) || []).map(Number));
        let pathKeyTimes = null;
        const pathKtAttr = child.getAttribute('keyTimes');
        if (pathKtAttr) {
            pathKeyTimes = pathKtAttr.split(';').map((s) => parseFloat(s.trim()));
        }
        let pathKeySplines = null;
        const pathKsAttr = child.getAttribute('keySplines');
        if (pathKsAttr) {
            pathKeySplines = pathKsAttr.split(';').map((s) => s.trim().split(/[\s,]+/).map(Number));
        }
        anims.push({
            keyframes,
            keyframeNums,
...
function parseAttributeAnimations(el) {
    const anims = [];
    for (let i = 0; i < el.children.length; i++) {
        const child = el.children[i];
        if (child.tagName !== 'animate')
        ...
        if (!attributeName || attributeName === 'd')
            continue;
        let values;
        const valuesAttr = child.getAttribute('values');
        if (valuesAttr) {
            values = valuesAttr.split(';').map((s) => s.trim()).filter(Boolean);
        }
        else {
...
function parseClipPathRef(attr) {
    if (!attr)
        return null;
    const m = attr.match(/url\(#([^)]+)\)/);
    return m ? m[1] : null;
}
function buildClipPath2D(clipEl) {
    ...
    for (let i = 0; i < clipEl.children.length; i++) {
        ...
        switch (tag) {
            ...
            case 'polygon': {
                ...
                if (pts)
                    combined.addPath(new Path2D(polygonPointsToD(pts)));
                break;
            }
        }
    }
 ...
 */
function compileElement(el, doc, inherited) {
    ...
    switch (tag) {
        ...
        case 'polygon': {
            ...
            if (pts) {
                try {
                    node.path2d = new Path2D(polygonPointsToD(pts));
                }
                catch {
                    node.path2d = null;
                }
...
function compileChildren(parent, doc, inherited) {
    const nodes = [];
...
function getAnimationSegment(valueCount, keyTimes, calcMode, keySplines, time, dur, begin) {
...
function interpolateTransformAnim(anim, time) {
...
function interpolateColor(from, to, lp) {
...
function interpolateAttributeAnim(anim, time) {
...
function buildAnimatedShapePath(node, time) {
        ...
        switch (node.tag) {
            ...
            case 'rect': {
                ...
                if (width <= 0 || height <= 0)
                    return null;
                if (rx > 0 || ry > 0)
                    path.roundRect(x, y, width, height, [rx || ry]);
                else
...
function interpolatePathAnim(anim, time) {
    ...
    if (nums1.length !== nums2.length || nums1.length === 0) {
        try {
            return new Path2D(anim.keyframes[idx]);
        }
        catch {
            return null;
        }
...
function applyAnimatedTransforms(ctx, anims, time) {
