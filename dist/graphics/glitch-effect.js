...
exports.glitchSeedFor = glitchSeedFor;
exports.drawBodyWithGlitch = drawBodyWithGlitch;
...
function ensureBuffers(side) {
    if (side > MAX_BUFFER_SIDE)
        return false;
    if (bodyCtx && tintCtx && bufferSide >= side)
        return true;
 ...
 */
function drawBodyWithGlitch(g, radius, seed, drawBody) {
    ...
    for (let i = 0; i < BAND_COUNT; i++) {
        ...
        if (roll < BAND_DROP_CHANCE)
            continue; // signal dropout — band missing
        let dx = 0;
        if (roll < BAND_DROP_CHANCE + BAND_SHIFT_CHANCE) {
            dx = (hash01(seed, bucket * 17 + i) - 0.5) * radius * 0.9;
        }
        mainCtx.drawImage(bodyCanvas, 0, sy, side, sh, -half + dx, -half + sy * invScale, fullW, sh * invScale);
    }
