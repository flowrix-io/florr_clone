...
core_1.Graphics.prototype.drawFlower = function (attrs) {
    const ctx = this.ctx;
    ...
    if (faceFlags & core_1.FaceFlags.Poisoned) {
        ...
        baseColor = core_1.Graphics.mixColors(baseColor, '#ffffff', 0.4);
    }
    ...
    const strokeColor = core_1.Graphics.darkenColor(baseColor, 0.8);
    // Outer border circle
    ...
    else if (faceFlags & core_1.FaceFlags.SquareEyes) {
    ...
    if (!(faceFlags & core_1.FaceFlags.DeadEyes) && mouth <= 8 && (faceFlags & core_1.FaceFlags.Attacking)) {
        ctx.save();
    ...
    if (!(faceFlags & core_1.FaceFlags.DeadEyes) && mouth <= 8 && (faceFlags & core_1.FaceFlags.Defending)) {
        ctx.save();
        ctx.fillStyle = baseColor;
        ctx.translate(0, -mouth - 8);
