...
class Game {
    ...
    constructor(showHitboxes, serverIp, preloadedAssets, showStats = false, dynamicSkybox = false) {
        ...
        this.mobileControlsEnabled = (0, mobile_controls_1.resolveMobileControlsEnabled)();
        this.mobileControls = new mobile_controls_1.MobileControls();
        this.mouseX = 0;
        this.mouseY = 0;
        ...
        this._irisCoveredFrameRendered = false;
        this.fpsCounter = 0;
        ...
        (0, app_refs_1.setCurrentGame)(this);
        ...
        this.canvas.addEventListener('mousedown', (event) => {
            ...
            if (event.button === 0 && debug_menu_1.debugMenuPanel.isMenuOpen()) {
                const { x: sx, y: sy } = (0, zoom_compensation_1.canvasCoords)(this.canvas, event, true);
                if (debug_menu_1.debugMenuPanel.isPointInside(sx, sy)) {
        ...
        const touchPoints = (touches) => {
            ...
            for (let i = 0; i < touches.length; i++) {
                const t = touches[i];
                const { x, y } = (0, zoom_compensation_1.canvasCoords)(this.canvas, t, true);
      ...
      `;
        ...
        this.socket.on('debugStats', (stats) => {
            debug_menu_1.debugMenuPanel.recordServerStats(stats);
        });
    ...
    async performAuthentication() {
        ...
        const credentials = await (0, auth_session_1.getSocketAuth)(playerName, spawnBiome);
        if (!credentials) {
    ...
    setupSettingsListeners() {
        ...
        const pollSettings = () => {
            if ((0, app_refs_1.getCurrentGame)() !== this)
                return;
    ...
    frame() {
        ...
        const titleScreen = (0, app_refs_1.getTitleScreen)();
        ...
        if (titleScreen) {
            titleScreen.getCanvasButtons().setDebugVisible((0, debug_menu_1.isDebugMenuEnabled)());
        }
        debug_menu_1.debugMenuPanel.render(this.graphics.ctx);
        if (this.showStats) {
    ...
    isAnyMenuOpen() {
        ...
        if ((0, app_refs_1.getTitleScreen)()?.isSettingsOpen()) {
            return true;
        }
    ...
    cleanup() {
        ...
        if (this.socket) {
            if (this.socket.connected) {
                ...
                (0, preconnect_1.reuseSocketForTitleScreen)(this.socket);
            }
            else {
