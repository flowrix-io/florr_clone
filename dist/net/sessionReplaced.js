...
exports.isSessionReplaced = isSessionReplaced;
exports.attachSessionReplacedHandler = attachSessionReplacedHandler;
const app_refs_1 = require("../app_refs");
let replaced = false;
/** True once this page has been signed out by a session elsewhere. */
function isSessionReplaced() {
    return replaced;
}
/**
 ...
 */
function attachSessionReplacedHandler(socket) {
    socket.on('sessionReplaced', (data) => {
        ...
        (0, app_refs_1.getCurrentGame)()?.suppressUnloadWarning();
        showSessionReplacedOverlay(data?.message);
    });
}
function showSessionReplacedOverlay(message) {
    if (document.getElementById('session-replaced-overlay'))
        return;
