...
exports.runPetalRingSelfTest = runPetalRingSelfTest;
const petalRing_1 = require("./petalRing");
function slot(petalType) {
...
function runPetalRingSelfTest() {
    ...
    // -- slot assignment: spread vs clumped -----------------------------------
    {
        ...
        const slotCount = (0, petalRing_1.layoutPetalRing)(loadout, statsOf, out);
        ...
        checkEqual('instance order is loadout-then-count', inst, '0:0 1:0 1:1 1:2 2:0 2:1 2:2 2:3 3:0');
    }
    ...
    // -- instance keys are injective -------------------------------------------
    {
        ...
        for (let s = 0; s < 10; s++) {
            for (let i = 0; i < 64; i++) {
                const key = (0, petalRing_1.petalInstanceKey)(s, i);
                if (seen.has(key))
                    collisions++;
                seen.add(key);
            }
    ...
    // -- dropSlot hits exactly one slot ----------------------------------------
    {
        ...
        const ring = new petalRing_1.PetalRingState();
    ...
    // -- the orbit phase is an integral ----------------------------------------
    {
        ...
        let phase = 0;
        for (let i = 0; i < 3; i++)
            phase = (0, petalRing_1.advanceOrbitPhase)(phase, 1, dt);
        phase = (0, petalRing_1.advanceOrbitPhase)(phase, 2, dt);
    ...
    // -- position modes and state acquisition ----------------------------------
    {
        ...
        const result = { x: 0, y: 0, angle: 0, homing: false };
        const target = { x: 0, y: 0, angle: 0, range: 0 };
        const modes = [
        ...
        for (let m = 0; m < modes.length; m++) {
            const ring = new petalRing_1.PetalRingState();
            const mode = modes[m];
            (0, petalRing_1.stepPetalKinematics)(ring, geom, mode.stats, m, 0, m, 1, INERT_DEPS, result);
            ...
            if (mode.name === 'noPhysics') {
                ...
                (0, petalRing_1.petalOrbitTarget)(geom, mode.stats, m, 0, 1, target);
    ...
    // -- clumped instances cluster, spread instances do not ---------------------
    {
        ...
        const a = { x: 0, y: 0, angle: 0, range: 0 };
        const b = { x: 0, y: 0, angle: 0, range: 0 };
        // Two instances of a CLUMPED petal share slot 0 but must not coincide.
        (0, petalRing_1.petalOrbitTarget)(geom, TABLE.clump, 0, 0, 1, a);
        (0, petalRing_1.petalOrbitTarget)(geom, TABLE.clump, 0, 1, 1, b);
        ...
        (0, petalRing_1.petalOrbitTarget)(geom, TABLE.three, 0, 0, 1, a);
        (0, petalRing_1.petalOrbitTarget)(geom, TABLE.three, 0, 1, 1, b);
    ...
    // -- defendOnly never extends on attack ------------------------------------
    {
        ...
        const make = (petalExtension) => (0, petalRing_1.computeRingGeometry)({
            playerX: 0,
            playerY: 0,
            orbitPhase: 0,
            slotCount: 1,
        ...
        checkEqual('attack does not extend defendOnly', attack.defendOnlyBaseRadius, neutral.defendOnlyBaseRadius);
        checkEqual('defend pulls the ordinary radius in', defend.baseRadius < neutral.baseRadius, true);
        checkEqual('defend also pulls defendOnly in', defend.defendOnlyBaseRadius, defend.baseRadius);
    }
    return failures;
}
