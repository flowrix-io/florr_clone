...
exports.runCombatSelfTest = runCombatSelfTest;
...
function runCombatSelfTest() {
    ...
    function addMob(world, id, x, y, radius = 20, damage = 5, health = 100) {
    ...
    // -- a single shot -------------------------------------------------------
    {
        ...
        checkClose('inherits the aim angle', world.get(p, C.Angle, 'value'), 0);
        // Config speed is px/sec; flight is px/ms.
        checkClose('speed converted to px/ms', world.get(p, C.Speed, 'current'), 0.4, 1e-7);
        // distance * (SIZE_SCALING / 9) = 900 * 1
        checkClose('travel distance scaled by rarity', world.get(p, C.Projectile, 'maxDistance'), 900, 1e-3);
        // size * (SIZE_SCALING / 3) = 2 * 3
        checkClose('size scaled by rarity', world.get(p, C.Projectile, 'size'), 6, 1e-3);
    ...
    // -- cooldown gating ------------------------------------------------------
    {
        ...
        fire(shooter, 0, 11000); // inside the cooldown
        checkEqual('second volley suppressed by cooldown', world.query([C.IsProjectile]).count(), 1);
        fire(shooter, 0, 12100); // past it
        checkEqual('volley fires again once the cooldown elapses', world.query([C.IsProjectile]).count(), 2);
    }
    // -- multi-shot fan is centred on the aim ---------------------------------
    {
        ...
        fire(shooter, 1.0, 10000);
        const angles = world.query([C.IsProjectile]).collect()
            .map(p => world.get(p, C.Angle, 'value'))
            .sort((a, b) => a - b);
    ...
    // -- no config, no shot ----------------------------------------------------
    {
        ...
        fire(shooter, 0, 10000);
        checkEqual('a mob with no projectile config never fires', world.query([C.IsProjectile]).count(), 0);
    }
    // -- a dead shooter does not fire -------------------------------------------
    {
        ...
        fire(shooter, 0, 10000);
        checkEqual('a destroyed shooter fires nothing', world.query([C.IsProjectile]).count(), 0);
    }
    function makeCollision(noCollisionFor = new Set()) {
        const world = new world_1.World();
        const scheduler = new system_1.Scheduler(world);
        const events = { damaged: [], killed: [], credits: [] };
        const deps = {
            resolveWall: (x, y) => ({ x, y }),
    ...
    // -- overlapping mobs push apart --------------------------------------------
    {
        ...
        h.tick();
        const ax = h.world.get(a, C.Position, 'x');
        const bx = h.world.get(b, C.Position, 'x');
    ...
    // -- separation is capped per tick --------------------------------------------
    {
        ...
        h.tick();
        const moved = Math.abs(h.world.get(a, C.Position, 'x'));
    ...
    // -- different chains DO push ------------------------------------------------------
    {
        ...
        h.world.add(headA, C.CentipedeSegment, { leader: entity_1.NULL_ENTITY, head: headA, segmentIndex: 0 });
        h.world.add(headB, C.CentipedeSegment, { leader: entity_1.NULL_ENTITY, head: headB, segmentIndex: 0 });
        h.tick();
        check('separate chains still collide', h.world.get(headB, C.Position, 'x') > 10);
    }
    ...
    {
        const h = makeCollision();
        const owner = (0, prefabs_1.spawnPlayer)(h.world, {
    ...
    // -- pet vs pet does no damage -----------------------------------------------------------
    {
        ...
        (0, prefabs_1.makePet)(h.world, petA, owner);
        (0, prefabs_1.makePet)(h.world, petB, owner);
        h.tick();
