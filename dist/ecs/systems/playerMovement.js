...
exports.createPlayerMovementQueries = createPlayerMovementQueries;
exports.playerMovementSystem = playerMovementSystem;
exports.registerPlayerMovementSystem = registerPlayerMovementSystem;
...
function createPlayerMovementQueries(world) {
    return {
        ...
        movable: world.query([C.Position, C.Velocity, C.Angle, C.PlayerInput, C.PlayerModifiers, C.IsPlayer], [C.IsDead, C.IsLobby]),
    };
}
/**
 ...
 */
function playerMovementSystem(queries, deps) {
        ...
        queries.movable.chunks(chunk => {
            ...
            for (let i = 0; i < chunk.count; i++) {
                ...
                if (input.useMouse[i]) {
                    ...
                    const keys = input.keys[i];
                    if (held(keys, KEYS_LEFT))
                        targetVX -= 1;
                    if (held(keys, KEYS_RIGHT))
                        targetVX += 1;
                    if (held(keys, KEYS_UP))
                        targetVY -= 1;
                    if (held(keys, KEYS_DOWN))
                        targetVY += 1;
...
function registerPlayerMovementSystem(scheduler, queries, deps) {
    scheduler.add('playerMovement', system_1.Phase.Simulation, playerMovementSystem(queries, deps));
}
