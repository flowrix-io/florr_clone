...
exports.MAX_WANDER_STEP = exports.WANDER_REF_RADIUS = exports.ENEMY_SPEED_MULTIPLIER = void 0;
exports.createEnemyPassiveQueries = createEnemyPassiveQueries;
exports.defaultPassiveSystem = defaultPassiveSystem;
exports.beePassiveSystem = beePassiveSystem;
exports.registerEnemyPassiveSystems = registerEnemyPassiveSystems;
const C = __importStar(require("../components"));
const system_1 = require("../system");
/** Multiplier applied to a mob's stat speed throughout the AI. */
exports.ENEMY_SPEED_MULTIPLIER = 2;
/**
 ...
 */
exports.MAX_WANDER_STEP = 300 / 30;
/** gardn's friction, recalibrated from 1/3 @ 20 TPS to this server's 30 TPS. */
const FRICTION = 0.25;
function createEnemyPassiveQueries(world) {
...
function sizeFactor(radius) {
    return radius / exports.WANDER_REF_RADIUS;
}
/**
 ...
 */
function integrate(pos, vel, i, accelX, accelY) {
    ...
    if (magnitude > exports.MAX_WANDER_STEP) {
        const k = exports.MAX_WANDER_STEP / magnitude;
        vx *= k;
        vy *= k;
    }
    vel.x[i] = vx;
 ...
 */
function defaultPassiveSystem(queries) {
    return (ctx) => {
        const now = ctx.now;
        queries.defaultPassive.chunks(chunk => {
            ...
            for (let i = 0; i < chunk.count; i++) {
                ...
                const accel = speed.current[i] * exports.ENEMY_SPEED_MULTIPLIER * 0.25 * sizeFactor(radius.value[i]);
                let accelX = 0;
                let accelY = 0;
                const elapsed = now - passive.stateStart[i];
                if (passive.state[i] === 0 /* C.PassiveState.Idle */) {
                    if (elapsed >= 1000) {
                        angle.value[i] = Math.random() * Math.PI * 2;
                        passive.state[i] = 1 /* C.PassiveState.Moving */;
                        passive.stateStart[i] = now;
                    }
                }
                else {
                    if (elapsed >= 2500) {
                        passive.state[i] = 0 /* C.PassiveState.Idle */;
                        passive.stateStart[i] = now;
                    }
                    else if (elapsed >= 500) {
 ...
 */
function beePassiveSystem(queries) {
    return (ctx) => {
        const now = ctx.now;
        queries.beePassive.chunks(chunk => {
            ...
            for (let i = 0; i < chunk.count; i++) {
                ...
                if ((t * 1000) % 1500 < 500)
                    magnitude *= 0.5;
                integrate(pos, vel, i, Math.cos(angle.value[i]) * magnitude, Math.sin(angle.value[i]) * magnitude);
            }
        });
    };
}
function registerEnemyPassiveSystems(scheduler, queries) {
    scheduler.add('defaultPassive', system_1.Phase.Simulation, defaultPassiveSystem(queries));
    scheduler.add('beePassive', system_1.Phase.Simulation, beePassiveSystem(queries));
}
