...
exports.MOB_KNOCKBACK_FORCE = exports.PLAYER_KNOCKBACK_FORCE = void 0;
exports.createProjectileCollisionQueries = createProjectileCollisionQueries;
exports.projectileCollisionSystem = projectileCollisionSystem;
exports.registerProjectileCollisionSystem = registerProjectileCollisionSystem;
...
exports.PLAYER_KNOCKBACK_FORCE = 25;
/** Knockback applied to a MOB hit by a projectile, before mass division. */
exports.MOB_KNOCKBACK_FORCE = 20;
/** Mass assumed for a mob whose config could not be resolved. */
const DEFAULT_MOB_MASS = 1.0;
function createProjectileCollisionQueries(world) {
    return {
        mobProjectiles: world.query([C.Position, C.Radius, C.Health, C.Damage, C.Projectile, C.ProjectileOrigin], [C.FromPlayer, C.IsDead]),
        playerProjectiles: world.query([C.Position, C.Radius, C.Health, C.Damage, C.Projectile, C.ProjectileOrigin, C.FromPlayer], [C.IsDead]),
        players: world.query([C.Position, C.IsPlayer], [C.IsDead, C.IsLobby]),
    };
}
function makeProjEntry() {
    return {
        entity: entity_1.NULL_ENTITY, x: 0, y: 0, radius: 0, damage: 0, health: 0,
        petalType: 0, petalRarity: 0, distance: 0, maxDistance: 0,
        shooter: entity_1.NULL_ENTITY, sourceType: 0, consumed: false,
    };
}
/** Sentinel `sourceType` meaning "no mob type was stamped" (see prefabs.ts). */
const NO_SOURCE_TYPE = 0xffff;
function projectileCollisionSystem(queries, grid, gridResult, deps) {
    const { petalDamageOf, massOf, playerRadiusOf, damageMultiplierOf, onPlayerHit, creditDamage, emitEnemyDamaged, markEnemyDamaged, onProjectileKill, } = deps;
    ...
    function gather(query, pool) {
        ...
        query.chunks(chunk => {
            ...
            for (let i = 0; i < chunk.count; i++) {
                ...
                entry.shooter = origin.shooter[i];
    ...
    function lookedUpDamage(entry) {
        const stats = petalDamageOf(interning_1.petalTypes.nameOf(entry.petalType), entry.petalRarity);
     ...
     */
    function damageMob(world, victim, amount, dx, dy, distance, killer, timing, immediateBroadcast) {
        const current = world.get(victim, C.Health, 'current');
        const next = Math.max(0, current - amount);
        world.set(victim, C.Health, 'current', next);
        if (immediateBroadcast)
            emitEnemyDamaged(victim, next);
        else
        ...
        if (distance > 0) {
            ...
            const effective = exports.MOB_KNOCKBACK_FORCE / mass;
    ...
    return (ctx) => {
        ...
        for (let i = 0; i < mobCount; i++) {
            ...
            if (!isPet) {
                for (let j = 0; j < playerCount; j++) {
                    ...
                    if (!(distance < target.radius + p.radius))
                        continue;
                    let knockbackX = 0;
                    let knockbackY = 0;
                    if (distance > 0) {
                        knockbackX = (dx / distance) * exports.PLAYER_KNOCKBACK_FORCE;
                        knockbackY = (dy / distance) * exports.PLAYER_KNOCKBACK_FORCE;
                    }
                    const stillAlive = onPlayerHit(target.entity, 
                    // The player path uses the STAMPED damage, unlike the
                    // three paths that re-look it up. Preserved as-is.
                    p.damage, knockbackX, knockbackY, p.sourceType === NO_SOURCE_TYPE ? '' : interning_1.mobTypes.nameOf(p.sourceType));
                    if (!stillAlive)
                        target.dead = true;
                    p.consumed = true;
                ...
                const owner = world.get(p.shooter, C.PetOwner, 'owner');
                ...
                for (let k = 0; k < gridResult.count; k++) {
                    ...
                    const distance = Math.sqrt(dx * dx + dy * dy);
                    if (!(distance < gridResult.radius[k] + p.radius))
                        continue;
                    ...
                    if (owner !== entity_1.NULL_ENTITY && world.isAlive(owner)) {
        ...
        for (let i = 0; i < playerProjCount; i++) {
            ...
            for (let j = 0; j < mobCount; j++) {
                ...
                if (!(distance < p.radius + m.radius && distance > 0))
                    continue;
            ...
            for (let k = 0; k < gridResult.count; k++) {
                ...
                if (world.has(victim, C.IsDead))
                    continue;
                damageMob(world, victim, amount, dx, dy, distance, p.shooter, 'sync-snapshot', false);
                p.consumed = true;
                cmd.destroy(p.entity);
                break;
            }
        ...
        for (let i = 0; i < mobCount; i++)
            mobPool[i].shooter = entity_1.NULL_ENTITY;
        for (let i = 0; i < playerProjCount; i++)
            playerPool[i].shooter = entity_1.NULL_ENTITY;
        for (let i = 0; i < playerCount; i++)
            playerEntries[i].entity = entity_1.NULL_ENTITY;
    };
}
function registerProjectileCollisionSystem(scheduler, queries, grid, gridResult, deps) {
    ...
    scheduler.add('projectileCollision', system_1.Phase.Combat, projectileCollisionSystem(queries, grid, gridResult, deps));
}
