...
class Cell {
    constructor() {
        this.handles = new Float64Array(INITIAL_CELL_CAPACITY);
        /**
         ...
         */
        this.indices = new Uint32Array(INITIAL_CELL_CAPACITY);
        this.x = new Float64Array(INITIAL_CELL_CAPACITY);
        this.y = new Float64Array(INITIAL_CELL_CAPACITY);
        this.radius = new Float32Array(INITIAL_CELL_CAPACITY);
        this.count = 0;
        this.capacity = INITIAL_CELL_CAPACITY;
        /** Rebuild index this cell last held anything, for pruning. */
        this.lastUsed = 0;
    }
    push(handle, index, px, py, r) {
    ...
    grow() {
        ...
        const handles = new Float64Array(capacity);
...
class GridQueryResult {
    ...
    entity(i) {
        ...
        while (next < capacity)
            next *= 2;
        const handles = new Float64Array(next);
        handles.set(this.handles);
        const x = new Float64Array(next);
        x.set(this.x);
        const y = new Float64Array(next);
        y.set(this.y);
        const radius = new Float32Array(next);
        radius.set(this.radius);
        this.handles = handles;
        this.x = x;
        this.y = y;
...
exports.GridQueryResult = GridQueryResult;
function key(cx, cy) {
...
class SpatialGrid {
    constructor() {
         ...
         */
        this.stamps = new Uint32Array(1024);
        this.queryStamp = 0;
        this.rebuildCount = 0;
        /**
         ...
         */
        this.activeCells = [];
        this.loggedBadRadius = NaN;
        this.loggedBadQuery = NaN;
    }
    /**
        ...
        source.chunks(chunk => {
            ...
            for (let i = 0; i < chunk.count; i++) {
                ...
                if (!Number.isFinite(px) || !Number.isFinite(py))
                    continue;
                if (px > MAX_SANE_WORLD_COORD || px < -MAX_SANE_WORLD_COORD)
                    continue;
                if (py > MAX_SANE_WORLD_COORD || py < -MAX_SANE_WORLD_COORD)
                    continue;
                let r = rad.value[i];
                ...
                const index = (0, entity_1.entityIndex)(handle);
                const minCX = Math.floor((px - r) / exports.CELL_SIZE);
                const maxCX = Math.floor((px + r) / exports.CELL_SIZE);
                const minCY = Math.floor((py - r) / exports.CELL_SIZE);
                const maxCY = Math.floor((py + r) / exports.CELL_SIZE);
    ...
    query(x, y, radius, out) {
        ...
        const minCX = Math.floor((x - radius) / exports.CELL_SIZE);
        const maxCX = Math.floor((x + radius) / exports.CELL_SIZE);
        const minCY = Math.floor((y - radius) / exports.CELL_SIZE);
        const maxCY = Math.floor((y + radius) / exports.CELL_SIZE);
        for (let cy = minCY; cy <= maxCY; cy++) {
            for (let cx = minCX; cx <= maxCX; cx++) {
    ...
    ensureStampCapacity(slotCount) {
        if (slotCount <= this.stamps.length)
            return;
