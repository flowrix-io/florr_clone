...
const Position = (0, component_1.defineComponent)('TestPosition', { x: 'f64', y: 'f64' });
const Velocity = (0, component_1.defineComponent)('TestVelocity', { x: 'f32', y: 'f32' });
const Health = (0, component_1.defineComponent)('TestHealth', { current: 'f32', max: 'f32' });
const Payload = (0, component_1.defineComponent)('TestPayload', { data: 'obj', label: 'str' });
const IsPet = (0, component_1.defineTag)('TestIsPet');
function runEcsSelfTest() {
    const failures = [];
    ...
    // -- handle packing -------------------------------------------------------
    {
        const e = (0, entity_1.makeEntity)(1234, 5678);
        checkEqual('handle index roundtrip', (0, entity_1.entityIndex)(e), 1234);
        checkEqual('handle generation roundtrip', (0, entity_1.entityGeneration)(e), 5678);
        ...
        const big = (0, entity_1.makeEntity)(16777215, 100000000);
        checkEqual('handle index roundtrip (max)', (0, entity_1.entityIndex)(big), 16777215);
        checkEqual('handle generation roundtrip (large)', (0, entity_1.entityGeneration)(big), 100000000);
    ...
    // -- lifecycle and stale handles -----------------------------------------
    {
        ...
        checkEqual('slot is recycled', (0, entity_1.entityIndex)(b), (0, entity_1.entityIndex)(a));
        check('recycled slot has a new generation', (0, entity_1.entityGeneration)(b) !== (0, entity_1.entityGeneration)(a));
    ...
    // -- reference fields are released on destroy (heap-leak guard) -----------
    {
        ...
        checkEqual('obj field readable', world.get(entities[3], Payload, 'data').big[0], 3);
        ...
        for (const archetype of world.archetypes) {
            ...
            if (!col)
                continue;
            const data = col.arrays.data;
            const label = col.arrays.label;
    ...
    // -- deferred structural changes during iteration --------------------------
    {
        ...
        let visited = 0;
        scheduler.add('reap', system_1.Phase.Lifetime, ctx => {
            mortal.chunks(chunk => {
                ...
                for (let i = 0; i < chunk.count; i++) {
                    ...
                    if (h.current[i] < 10)
                        ctx.cmd.destroy(chunk.entities[i]);
                }
            });
        });
    ...
    // -- scheduler ordering and striding ---------------------------------------
    {
        ...
        scheduler.add('net', system_1.Phase.Networking, () => { order.push('net'); });
        scheduler.add('sim', system_1.Phase.Simulation, () => { order.push('sim'); });
        scheduler.add('grid', system_1.Phase.SpatialIndex, () => { order.push('grid'); });
        scheduler.add('strided', system_1.Phase.Lifetime, () => { stridedCalls++; }, { interval: 5, offset: 2 });
        scheduler.tick(1 / 30, 1000 / 30, 0);
        ...
        let dupThrew = false;
        try {
            scheduler.add('sim', system_1.Phase.Simulation, () => { });
        }
        catch {
            dupThrew = true;
        }
    ...
    // -- f64 world coordinates hold up at maze offsets -------------------------
    {
        ...
        const moved = world.get(e, Position, 'x') - 200000;
        check('f64 position accumulates sub-pixel steps at maze offset', Math.abs(moved - 1) < 1e-6, `drifted to ${moved}`);
    ...
    // -- the game component catalog -------------------------------------------
    {
        ...
        const components = (0, component_1.allComponents)();
        for (let i = 0; i < components.length; i++) {
            const c = components[i];
            if (c.id !== i)
                denseThroughout = false;
            seenIds.add(c.id);
        ...
        checkEqual('rarity id of common', (0, interning_1.rarityToId)('common'), 0);
        checkEqual('rarity roundtrip', (0, interning_1.idToRarity)((0, interning_1.rarityToId)('legendary')), 'legendary');
        checkEqual('unknown rarity is -1', (0, interning_1.rarityToId)('not_a_rarity'), -1);
        // Interned ids are stable within the process and distinct per string.
        const bee = interning_1.mobTypes.intern('bee');
        const hornet = interning_1.mobTypes.intern('hornet');
        check('interned ids are distinct', bee !== hornet);
        checkEqual('interning is idempotent', interning_1.mobTypes.intern('bee'), bee);
        checkEqual('interner resolves names', interning_1.mobTypes.nameOf(hornet), 'hornet');
        checkEqual('unknown string is -1', interning_1.mobTypes.idOf('not_a_mob'), -1);
    }
    // -- composing realistic game entities ------------------------------------
    {
        ...
        world.add(beeEntity, C.MobKind, { type: interning_1.mobTypes.intern('bee'), tier: (0, interning_1.rarityToId)('rare') });
        world.add(beeEntity, C.MobAI, {
            aiType: 1 /* C.AiType.Neutral */,
            isChasing: 0,
        ...
        checkEqual('bee tier reads back as rare', (0, interning_1.idToRarity)(world.get(beeEntity, C.MobKind, 'tier')), 'rare');
        checkEqual('bee type reads back as bee', interning_1.mobTypes.nameOf(world.get(beeEntity, C.MobKind, 'type')), 'bee');
        ...
        checkEqual('bee keeps its speed after slow removal', world.get(beeEntity, C.Speed, 'base'), 90);
