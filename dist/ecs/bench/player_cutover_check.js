...
exports.runPlayerCutoverCheck = runPlayerCutoverCheck;
exports.main = main;
 ...
 */
function legacyMove(state, inputs, speedBoost, sizeMultiplier, deltaTime) {
    let targetVelocityX = 0;
    let targetVelocityY = 0;
    let speedFactor = speedBoost;
    if (!(speedFactor >= 0))
        speedFactor = 1; // NaN / negative -> 1
    if (speedFactor > 8)
        speedFactor = 8;
    ...
    if (inputs.useMouse &&
        ...
        const speed = constants_1.MAX_SPEED * speedFactor * mouseMult;
        ...
        if (inputs.keys.includes('ArrowLeft') || inputs.keys.includes('a'))
            targetVelocityX -= 1;
        if (inputs.keys.includes('ArrowRight') || inputs.keys.includes('d'))
            targetVelocityX += 1;
        if (inputs.keys.includes('ArrowUp') || inputs.keys.includes('w'))
            targetVelocityY -= 1;
        if (inputs.keys.includes('ArrowDown') || inputs.keys.includes('s'))
            targetVelocityY += 1;
        ...
        const speed = constants_1.MAX_SPEED * speedFactor;
    ...
    const effectivePlayerSize = constants_1.PLAYER_SIZE * (sizeMultiplier ?? 1.0);
    const moved = (0, constants_1.stepPlayerMovement)({ x: state.x, y: state.y, vx: state.velocityX, vy: state.velocityY }, targetVelocityX, targetVelocityY, deltaTime, effectivePlayerSize);
...
const STYLES = ['mouse', 'keys', 'idle', 'malformed-mouse', 'huge-boost'];
/**
 ...
 */
function makePlayer(id, x, y, sizeMultiplier) {
    return {
        id,
        name: id,
        x,
 ...
 */
function driveInputs(subject, tick, rng) {
    ...
    switch (style) {
        ...
        case 'keys': {
            const keys = [];
            if (rng() < 0.5)
                keys.push(rng() < 0.5 ? 'a' : 'ArrowLeft');
            if (rng() < 0.5)
                keys.push(rng() < 0.5 ? 'd' : 'ArrowRight');
            if (rng() < 0.5)
                keys.push('w');
            if (rng() < 0.5)
                keys.push('s');
...
function runPlayerCutoverCheck() {
    ...
    for (let tick = 0; tick < TICKS; tick++) {
        ...
        for (const s of subjects) {
            if (s.player.isDead)
                continue;
            legacyMove(s.reference, s.player.inputs, speedBoostOf(s.player), s.sizeMultiplier, DT);
            const p = s.player;
            const r = s.reference;
        ...
        for (const s of subjects) {
            if (s.player.isDead)
                continue;
            s.player.x = s.player.movedX;
            s.player.y = s.player.movedY;
        }
    ...
    let prefixBots = 0;
    for (const s of subjects) {
        if (s.player.id.startsWith('bot_'))
            prefixBots++;
        const entity = runtime.world.lookup(s.player.id);
        if (entity !== undefined && runtime.world.has(entity, C.IsBot))
            taggedBots++;
    }
