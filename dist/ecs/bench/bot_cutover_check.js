...
exports.runBotCutoverCheck = runBotCutoverCheck;
exports.main = main;
...
const botReach_1 = require("../../server/bots/botReach");
...
function makeIoStub() {
    const emits = [];
    const io = {
        emit: (event) => { emits.push({ room: null, event }); },
        to: (room) => ({
            emit: (event) => { emits.push({ room, event }); },
        }),
...
function petal(petalType, rarity, customSize) {
 ...
 */
function makeBot(id, x, y, loadout) {
    return {
        id,
        name: id,
        x,
...
function commitStagedPositions() {
    for (const id in constants_1.players) {
        const player = constants_1.players[id];
        if (player.movedX !== undefined)
            player.x = player.movedX;
        if (player.movedY !== undefined)
            player.y = player.movedY;
    }
}
function snapshotInputs(player) {
...
function snapshotComponent(runtime, id) {
    const entity = runtime.world.lookup(id);
    if (entity === undefined)
        return null;
    const world = runtime.world;
    return {
        useMouse: !!world.get(entity, C.PlayerInput, 'useMouse'),
        dirX: world.get(entity, C.PlayerInput, 'mouseDirectionX'),
        dirY: world.get(entity, C.PlayerInput, 'mouseDirectionY'),
        speedMult: world.get(entity, C.PlayerInput, 'mouseSpeedMultiplier'),
        petalExtension: world.get(entity, C.PlayerInput, 'petalExtension'),
    };
}
 ...
 */
function runOrderedTicks(runtime, io, tickCount, misplaceInput) {
    let stale = 0;
    let observed = 0;
    let moved = 0;
    for (let tick = 0; tick < tickCount; tick++) {
        ...
        if (misplaceInput) {
            ...
            (0, botManager_1.updateBotAI)(io, runtime.world, now);
            for (const id in constants_1.players)
                decided[id] = snapshotInputs(constants_1.players[id]);
        }
        for (const id in constants_1.players) {
            ...
            if (!component)
                continue;
            observed++;
            if (!sameInputs(component, decided[id]))
                stale++;
        }
        const before = {};
        for (const id in constants_1.players)
            before[id] = { x: constants_1.players[id].x, y: constants_1.players[id].y };
        runtime.tickPlayers(DT, DT * 1000, now);
        (0, ecsSync_1.syncPlayersFromEcs)(runtime.world, constants_1.players);
        commitStagedPositions();
        for (const id in constants_1.players) {
            const dx = constants_1.players[id].x - before[id].x;
            const dy = constants_1.players[id].y - before[id].y;
            if (dx * dx + dy * dy > 0.01)
                moved++;
        }
    }
...
function checkInputOrder() {
    ...
    (0, botManager_1.registerBotInputSystem)(runtime.inputScheduler, io);
    ...
    const inputIdentity = {};
    for (const id in constants_1.players)
        inputIdentity[id] = constants_1.players[id].inputs;
    ...
    for (const id in constants_1.players) {
        if (constants_1.players[id].inputs !== inputIdentity[id]) {
 ...
 */
function measuredReach(bot, petalExtension) {
    const instances = [];
    const slotCount = (0, petalRing_1.layoutPetalRing)(bot.loadout, ringStatsOf, instances);
    const out = { x: 0, y: 0, angle: 0, range: 0 };
    let farthest = 0;
    const STEPS = 1440;
    for (let step = 0; step < STEPS; step++) {
        const geom = (0, petalRing_1.computeRingGeometry)({
            ...
            playerSize: 40,
            rangeModifier: (0, botReach_1.botRangeModifier)(bot),
        ...
        for (const instance of instances) {
            ...
            (0, petalRing_1.petalOrbitTarget)(geom, stats, instance.slotIndex, instance.instanceIndex, effectiveSize, out);
 ...
 */
function legacyReach(bot, petalExtension) {
    const sizeMult = bot.sizeMultiplier ?? 1.0;
    const baseRadius = (60 + (40 / 2) * (sizeMult - 1)) * petalExtension;
    const playerRangeMod = (0, botReach_1.botRangeModifier)(bot);
...
function findPetalWith(predicate, rarity) {
    const candidates = [
...
function checkReach() {
    ...
    for (const testCase of cases) {
        ...
        bot.sizeMultiplier = testCase.sizeMultiplier ?? 1;
        for (const extension of [0.5, 1.0, 2.0]) {
            const measured = measuredReach(bot, extension);
            const claimed = (0, botReach_1.botPetalReach)(bot, extension, 0);
...
function checkStandoffReaches() {
    ...
    const extension = 2.0;
    const reachWithBuffer = (0, botReach_1.botPetalReach)(bot, extension, STANDOFF_SAFETY_BUFFER);
    const measured = measuredReach(bot, extension);
...
function checkTagAndLoadout() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        constants_1.players[bot.id] = bot;
        (0, ecsSync_1.ensurePlayerEntity)(world, bot, NOW0);
        bots.push(bot);
    }
    ...
    constants_1.players[human.id] = human;
    (0, ecsSync_1.ensurePlayerEntity)(world, human, NOW0);
    const counts = (0, botManager_1.botRosterCounts)(world);
    ...
    if (victim === undefined) {
        ...
        const broken = (0, botManager_1.botRosterCounts)(world);
    ...
    if (corpse === undefined) {
        ...
        const withCorpse = (0, botManager_1.botRosterCounts)(world);
    ...
    if (entity === undefined) {
        ...
        bots[1].loadout[0] = swapped;
        const after = world.get(entity, C.Loadout, 'slots');
        if (after[0] !== swapped) {
