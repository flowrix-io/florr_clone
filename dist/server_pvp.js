...
io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (credentials) => {
        ...
        if (user) {
            ...
            const chatMessage = {
                sender: "SERVER",
                content: `${constants_1.players[socket.id].name} has joined the PVP server`,
                timestamp: Date.now()
            };
        ...
        if (player) {
            ...
            for (const enemy of constants_1.enemies) {
                ...
                if (newX < enemy.x + enemySize &&
                    ...
                    collision = true;
                    console.log(`[SERVER_PVP] Enemy collision detected: player(${newX.toFixed(1)}, ${newY.toFixed(1)}, ${constants_1.PLAYER_SIZE}x${constants_1.PLAYER_SIZE}) vs enemy(${enemy.x.toFixed(1)}, ${enemy.y.toFixed(1)}, ${enemySize}x${enemySize}) tier:${enemy.tier} id:${enemy.id}`);
                    if (!player.isInvulnerable) {
                        ...
                        const knockbackX = -normalizedDx * constants_1.KNOCKBACK_FORCE;
                        const knockbackY = -normalizedDy * constants_1.KNOCKBACK_FORCE;
            ...
            const clampedX = Math.max(0, Math.min(constants_1.PVP_WORLD_WIDTH - constants_1.PLAYER_SIZE, newX));
            const clampedY = Math.max(0, Math.min(constants_1.PVP_WORLD_HEIGHT - constants_1.PLAYER_SIZE, newY));
