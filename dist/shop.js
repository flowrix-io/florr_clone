...
class ShopManager {
    constructor(game) {
        ...
        this.animTarget = 0; // 0 = hidden, 1 = visible (drives whether draw() runs after closeShop)
        this.rafScheduled = false;
        this.lastFrame = 0;
        this.activeTab = 'shop';
        this.scrollOffsets = { shop: 0, challenges: 0 };
        this.contentHeights = { shop: 0, challenges: 0 };
        this.hover = null;
        this.regions = [];
        this.codeText = '';
        this.codeCaret = 0;
        this.codeFocused = false;
        this.caretAnchorMs = 0; // when caret last reset (so blink restarts on input)
        this.modal = null;
        this.starIcon = null;
    ...
    isShopOpenState() { return this.isShopOpen; }
    toggleShop() {
        if (this.isShopOpen)
            this.closeShop();
        else
    ...
    /* --------------------------- Internal setup --------------------------- */
    preloadStarIcon() {
        const icon = game_icons_net_icons_1.GAME_ICONS_NET_ICONS.find(i => i.name === 'stars');
        if (!icon)
            return;
    ...
    /* ------------------------------ Drawing ------------------------------ */
    draw(t) {
        if (!this.isShopOpen && this.animTarget === 0)
            return;
    ...
    drawHeader(cssW) {
        ...
        ctx.lineTo(cb.x + cb.w - 6, cb.y + cb.h - 6);
        ctx.moveTo(cb.x + cb.w - 6, cb.y + 6);
        ctx.lineTo(cb.x + 6, cb.y + cb.h - 6);
        ctx.stroke();
        this.regions.push({ ...cb, kind: 'close' });
    }
    drawCodeSection(cssW) {
        const ctx = this.ctx;
    ...
    drawShopItems(x, y, w, h) {
        ...
        for (const petalType of this.allPetalTypes) {
            ...
            for (const rarity of this.buyableRarities) {
                ...
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.fillText(formatPrice(price), cx + cardSize / 2, cy + cardSize - ITEM_PRICE_BAR_H / 2);
                this.regions.push({
    ...
    handlePaste(e) {
        ...
        // (chat, guild menu, etc.).
        if (!this.isShopOpen || !this.codeFocused || this.modal)
            return;
