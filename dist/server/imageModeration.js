...
exports.screenImageUrl = screenImageUrl;
exports.filterChatImages = filterChatImages;
...
function isPrivateHost(hostname) {
    ...
    if (!host.includes('.') && !host.includes(':'))
        return true; // bare hostname → intranet
    if (host.includes(':'))
...
function screenUrl(raw) {
    ...
    const hit = BLOCKED_URL_KEYWORDS.some(word => word.length > 4 ? haystack.includes(word) : haystack.includes(` ${word} `));
    if (hit) {
...
async function checkHeaders(url, signal) {
    let res;
    try {
        res = await fetch(url, {
            method: 'GET',
 ...
 */
async function checkProvider(url, signal) {
...
async function screenImageUrl(raw) {
    ...
    if (!verdict.ok || !url)
        return cacheVerdict(raw, verdict);
    const run = (async () => {
...
function consumeCooldown(username) {
    const key = username.toLowerCase();
    const now = Date.now();
    const last = lastImagePost.get(key) || 0;
    if (now - last < IMAGE_POST_COOLDOWN_MS)
        return false;
    if (lastImagePost.size > 5000) {
        for (const [name, at] of lastImagePost) {
            if (now - at > IMAGE_POST_COOLDOWN_MS * 10)
                lastImagePost.delete(name);
        }
    }
...
function refundCooldown(username) {
 ...
 */
async function filterChatImages(message, username) {
    const tags = message.match(IMG_TAG_RE);
    if (!tags || tags.length === 0)
        return { content: message, notices: [] };
    const notices = [];
