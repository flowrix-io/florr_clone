...
exports.beginEnemyTick = beginEnemyTick;
exports.repairSeveredCentipedeChains = repairSeveredCentipedeChains;
exports.propagateCentipedeChains = propagateCentipedeChains;
 ...
 */
const MAX_WANDER_STEP = constants_1.MAX_SPEED / 30;
/**
...
const MAX_TARGET_DISTANCE = constants_1.VIEWPORT_WIDTH * 5;
// ---------------------------------------------------------------------------
 ...
 */
function wanderSizeFactor(enemy) {
...
function chaseStepOf(enemy) {
    return PLAYER_SPEED_CHASERS.has(enemy.type)
        ? (constants_1.MAX_SPEED / 30) * speedScaleOf(enemy) // player base speed, per 30 TPS tick
 ...
 */
function computeOwnSegmentAvoidance(enemy) {
    const isCentipedeHead = ((0, server_utils_1.isCentipedeHeadType)(enemy.type) || (0, server_utils_1.isCentipedeBodyType)(enemy.type)) && !enemy.leaderId;
    if (!isCentipedeHead)
        return null;
    ...
    let ax = 0;
    let ay = 0;
    for (const seg of constants_1.enemies) {
        if (seg === enemy)
            continue;
        if (!(0, server_utils_1.isCentipedeBodyType)(seg.type))
            continue;
        if (seg.headId !== enemy.id)
            continue;
        if (seg.leaderId === enemy.id)
            continue;
 ...
 */
function stepAlong(enemy, dx, dy, distance, speed, currentTime) {
    ...
    if (enemy.type === 'bee') {
        ...
        if (enemy.wobblePhase === undefined)
 ...
 */
function pickWanderTargetIfStale(enemy, currentTime) {
...
function wanderStepOf(enemy, factor) {
 ...
 */
function fireProjectileVolley(enemy, mobStats, aimAngle, currentTime) {
    ...
    const petalStats = (0, petals_1.getPetalStats)(projectileConfig.petalType, projectileRarity);
    if (!petalStats)
        return;
    for (let i = 0; i < projectileCount; i++) {
        ...
        const distanceScale = (mobs_1.SIZE_SCALING[enemy.tier] || 1) / 9;
        const sizeScale = (mobs_1.SIZE_SCALING[enemy.tier] || 1) / 3;
        const projectile = {
            id: (0, gameState_1.allocateMobProjectileId)(),
            enemyId: enemy.id,
            x: enemy.x,
            y: enemy.y,
            startX: enemy.x,
...
function beginEnemyTick(currentTime) {
...
function repairSeveredCentipedeChains(ctx) {
    for (const enemy of constants_1.enemies) {
        ...
        while (true) {
            const follower = constants_1.enemies.find(e => e.leaderId === leader.id);
            if (!follower)
                break;
 ...
 */
function propagateCentipedeChains(ctx) {
    ...
    for (const head of centipedeHeads) {
        ...
        for (const segment of chain) {
            ...
            (0, physics_1.checkEnemyWallCollisions)(segment);
        }
    }
}
// ---------------------------------------------------------------------------
...
function stepEnemy(enemy, ctx) {
    ...
    if (enemy.knockbackX || enemy.knockbackY) {
        ...
        (0, physics_1.applyEnemyKnockback)(enemy);
        if (enemy.knockbackX && Math.abs(enemy.knockbackX) < 0.1)
            enemy.knockbackX = 0;
        if (enemy.knockbackY && Math.abs(enemy.knockbackY) < 0.1)
            enemy.knockbackY = 0;
    }
...
function stepPet(enemy, ctx) {
    const { currentTime } = ctx;
    const owner = constants_1.players[enemy.ownerId];
    ...
    if (owner && !owner.isDead) {
        if ((0, physics_1.hasLineOfSight)(enemy.x, enemy.y, owner.x, owner.y)) {
...
function followOwner(enemy, owner) {
 ...
 */
function teleportNearOwner(enemy, owner) {
    ...
    for (const angle of angles) {
        ...
        const isInWall = (0, constants_1.isTileIdBlocking)((0, constants_1.getTileState)(map_data_1.WALL_GRID, teleportX, teleportY));
    ...
    if (!(0, constants_1.isTileIdBlocking)((0, constants_1.getTileState)(map_data_1.WALL_GRID, owner.x, owner.y))) {
        enemy.x = owner.x;
        enemy.y = owner.y;
    }
}
/** Wander used by a pet whose owner is gone — straight at the target, no avoidance. */
function stepPetWander(enemy, currentTime) {
 ...
 */
function stepCentipedeHeadWander(enemy, currentTime) {
    const factor = pickWanderTargetIfStale(enemy, currentTime);
 ...
 */
function acquirePetWildTarget(enemy, ctx) {
    ...
    for (const otherEnemy of constants_1.enemies) {
...
function stepWildMob(enemy, ctx) {
    if (applyParentHoleTether(enemy, ctx))
        return;
 ...
 */
function applyParentHoleTether(enemy, ctx) {
    if (!enemy.parentHoleId)
        return false;
    const hole = ctx.enemyById.get(enemy.parentHoleId);
    if (!hole || hole.isDead || hole.health <= 0) {
 ...
 */
function acquirePlayerTarget(enemy, ctx) {
    let targetPlayer;
    if (enemy.targetPlayerId && constants_1.players[enemy.targetPlayerId]) {
        const existingTarget = constants_1.players[enemy.targetPlayerId];
 ...
 */
function acquirePetTarget(enemy, ctx, targetPlayer) {
    if (targetPlayer)
        return undefined;
    if (!(enemy.aiType === 'hostile' || (enemy.aiType === 'neutral' && enemy.targetPlayerId)))
        return undefined;
...
function chaseAndShoot(enemy, target, ctx) {
    const { currentTime } = ctx;
    enemy.isChasing = true;
...
function stepSandstorm(enemy, ctx) {
    const { currentTime } = ctx;
    enemy.isChasing = false;
    ...
    if ((0, petals_1.getRarityIndex)(enemy.tier) >= (0, petals_1.getRarityIndex)('super')) {
 ...
 */
function stepGardnPassive(enemy, currentTime) {
    const FRICTION = 0.25;
