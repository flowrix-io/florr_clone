...
exports.isBot = isBot;
exports.botRosterCounts = botRosterCounts;
 ...
 */
function rebuildBotRoster(world) {
    ...
    botQuery.chunks(chunk => {
        ...
        for (let i = 0; i < chunk.count; i++) {
            const id = world.externalIdOf(entities[i]);
            if (id === undefined)
                continue;
    ...
    for (const id in constants_1.players)
        if (isBot(id))
            prefixCount++;
    if (botRoster.length === prefixCount)
        return;
    if (rosterMismatchLogged < 5) {
        rosterMismatchLogged++;
        console.warn(`[bots] ECS roster (${botRoster.length}) disagrees with the bot_ prefix `
 ...
 */
function botRosterCounts(world) {
    if (botQueryWorld !== world || botQuery === undefined) {
        botQuery = world.query([C.IsPlayer, C.IsBot], [C.IsLobby]);
        botQueryWorld = world;
    }
    ...
    botQuery.chunks(chunk => {
        ...
        for (let i = 0; i < chunk.count; i++) {
            const id = world.externalIdOf(entities[i]);
            if (id !== undefined && constants_1.players[id] !== undefined)
                ecs++;
        }
    });
    let prefix = 0;
...
function pickFarmingZoneAnchor(bot, rarityIdx, rotation = 0) {
...
function hasNearbyBotBuddy(bot, range = YGGDRASIL_BUDDY_RANGE) {
...
function createBot(io, world) {
    ...
    (0, ecsSync_1.ensurePlayerEntity)(world, bot, Date.now());
    botAIState.set(id, {
        wanderTargetX: bot.x,
        wanderTargetY: bot.y,
        nextWanderTime: 0
...
function removeBot(id, io) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { cleanupPlayerPetalActionState } = require('../petal_actions');
 ...
 */
function maintainBotCount(io, realPlayerCount, world) {
    const now = Date.now();
...
function getBotSpeedMod(bot) {
    return (0, botReach_1.botSpeedModifier)(bot);
}
...
function computePetalReach(bot, petalExtension) {
    return (0, botReach_1.botPetalReach)(bot, petalExtension, STANDOFF_SAFETY_BUFFER);
}
...
function heapPop() {
    ...
    heapTx[0] = heapTx[heapSize];
    heapTy[0] = heapTy[heapSize];
    let i = 0;
    const n = heapSize;
    while (true) {
        const l = 2 * i + 1;
        const r = 2 * i + 2;
        let sm = i;
        if (l < n && heapF[l] < heapF[sm])
            sm = l;
        if (r < n && heapF[r] < heapF[sm])
            sm = r;
        if (sm === i)
            break;
        const tf = heapF[i];
...
let searchCells = 0;
function ensureSearchScratch(cells) {
    if (searchCells === cells)
        return;
...
function findPathAStar(startX, startY, goalX, goalY) {
    ...
    if (sx === gx && sy === gy)
        return [];
    ensureSearchScratch(gridW * map_data_1.WALL_GRID.length);
    beginSearch();
    ...
    while (heapSize > 0 && expanded < PATH_MAX_NODES) {
        ...
        for (let n = 0; n < A_STAR_NEIGHBORS.length; n++) {
            ...
            if (dx !== 0 && dy !== 0) {
                if (tileBlocked(curTx + dx, curTy))
                    continue;
                if (tileBlocked(curTx, curTy + dy))
                    continue;
            }
...
function pickBestEnemyTarget(bot, anchor, tetherRadius, preferredTiers, stickyId, stickiness = TARGET_STICKINESS) {
...
const FARM_ZONE_MIN_MS = 90000;
const FARM_ZONE_MAX_MS = 240000;
...
function computeBotMode(bot, groups, state, now) {
...
const botPersona = new Map();
function getBotPersona(id) {
...
const UNSTICK_ESCALATE_WINDOW_MS = 15000;
// Keep the watchdog quiet while the bot is deliberately standing still.
function resetOscillationSampler(bot, state, now) {
...
function detectOscillation(bot, state, now) {
    ...
    if (now - state.oscSampleAt < OSC_SAMPLE_MS)
        return false;
...
function pickUnstickDirection(bot, state) {
 ...
 */
function registerBotInputSystem(scheduler, io) {
    scheduler.add('botAI', ecs_1.Phase.Input, (ctx) => {
 ...
 * re-read the clock.
 */
function updateBotAI(io, world, now) {
