...
const WIRE_FIELDS = [...PUBLIC_FIELDS, ...OWN_FIELDS];
/**
 ...
 */
function sanitizePlayerForClient(player, includeMobKills = false) {
    const out = project(player, WIRE_FIELDS);
    if (includeMobKills && player.mobKills !== undefined)
        out.mobKills = player.mobKills;
    return out;
}
/**
 ...
 */
function sanitizePublicPlayerForClient(player) {
 ...
 */
function sanitizePlayersForClient(players, ownSocketId) {
    const out = {};
...
function project(player, fields) {
    const out = {};
