...
exports.isLocalSocket = isLocalSocket;
exports.kickDuplicateSessions = kickDuplicateSessions;
...
function expandIPv6(input) {
    ...
    if (halves.length > 2)
        return null;
    const toGroups = (s) => s === '' ? [] : s.split(':').map(g => (/^[0-9a-f]{1,4}$/.test(g) ? parseInt(g, 16) : NaN));
...
function describeAddress(socket) {
    if (socket.proxiedFor)
        return `${socket.proxiedFor} via ${socket.remoteAddress || 'proxy'}`;
...
function sessionOf(socketId) {
    return constants_1.players[socketId] || gameState_1.lobbyPlayers[socketId];
}
/**
 ...
 */
function kickDuplicateSessions(io, socket, userId, savePlayerProgressImmediate) {
    const incomingIsLocal = isLocalSocket(socket);
    let kicked = 0;
    for (const other of Array.from(io.sockets.sockets.values())) {
        ...
        const otherUserId = other.userId || gameState_1.playerUserIds[other.id];
        if (otherUserId !== userId)
            continue;
        // The multi-tab exemption: both ends of the pair have to be loopback.
        if (incomingIsLocal && isLocalSocket(other))
            continue;
        const otherSession = sessionOf(other.id);
        if (otherSession)
            savePlayerProgressImmediate(otherSession, userId);
