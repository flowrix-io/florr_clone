...
function resetPetalPhysicsOnBreak(playerId, loadoutIndex, instanceIndex) {
...
function resetSlotPetalPhysicsOnBreak(playerId, loadoutIndex) {
 ...
 */
function computeTargetVelocity(player) {
    ...
    let speedFactor = player.speed_boost * (0, petal_actions_1.getSpeedMultiplier)(player);
...
function dropFieldsOnExtension(opts) {
    const { player, io, petalInstances, playerOrbitPhase, angleStep, playerRangeModifier, defendOnlyBaseRadius, playerSizeMult, } = opts;
...
function updatePlayerState(player, deltaTime, deps) {
    ...
    const moved = (0, constants_1.stepPlayerMovement)({ x: player.x, y: player.y, vx: player.velocityX, vy: player.velocityY }, targetVelocityX, targetVelocityY, deltaTime, effectivePlayerSize);
    ...
    if (player.loadout) {
        ...
        const defendOnlyBaseRadius = (60 + (constants_1.PLAYER_SIZE / 2) * (playerSizeMult - 1)) * Math.min(petalExtension, 1.0);
        ...
        for (let idx = 0; idx < petalInstances.length; idx++) {
            ...
            if (petalStats.fixedDirection !== undefined) {
                ...
                if (playerPetalAttractionRadius > 0 && !burstHealHoming && !burstShieldHoming) {
                    ...
                    const attractionCandidates = (0, enemyGrid_1.queryEnemiesNear)(targetX, targetY, playerPetalAttractionRadius, _attractionQueryBuffer);
                ...
                if (closestEnemy) {
                    ...
                    if (!constants_1.enemies.some(e => e.id === releasedFrom)) {
            ...
            if (petalStats.wallCollide) {
                const resolved = (0, physics_1.checkPlayerWallCollisions)(petalX, petalY, 40 * effectiveSize);
