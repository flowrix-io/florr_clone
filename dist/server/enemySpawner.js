 ...
 */
function overlapsAnyMob(x, y, halfSize, extraGap = 0) {
    return (0, enemyRegistry_1.liveEnemies)().some((otherEnemy) => {
 ...
 */
function pickSectionMobType(x, y, weightTier, opts = {}) {
    ...
    const eligibleMobTypes = allMobTypes.filter(type => {
        if (type === 'target_dummy')
            return false;
        if (opts.excludeCentipedeBodies && (0, server_utils_1.isCentipedeBodyType)(type))
            return false;
        const stats = (0, mobs_1.getMobStats)(type, statsTier);
        return stats && stats.section.includes(currentSection);
    });
    if (eligibleMobTypes.length === 0)
        return null;
    return selectWeightedMobType(eligibleMobTypes, weightTier);
}
/**
 ...
 */
function selectBiomeSpawn(x, y, spawnTable, luckUpgradeBonus) {
 ...
 */
function finalizeMobSpawn(mobType, tier, x, y, reversed) {
    ...
    if (overlapsAnyMob(x, y, (mobStats.size * 40) / 2))
        return null;
...
function createEnemyInZone(helpers, zone) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (helpers.isPositionInPlayerPetalRange(x, y, PRELIMINARY_MOB_SIZE))
            continue;
        if (overlapsAnyMob(x, y, halfPrelimSize, MIN_MOB_SPAWN_DISTANCE))
            continue;
        validPosition = true;
        break;
    }
 ...
 */
function announceBossSpawn(tier, mob, mobSection, io) {
    if (mob.type === 'target_dummy')
        return;
    ...
    Object.entries(constants_1.players).forEach(([playerId, player]) => {
        ...
        io.to(playerId).emit('chatMessage', {
            sender: '',
            content: `<b style="color: ${constants_2.ENEMY_TIERS[tier].color};">A ${tier} ${mobName} has spawned${somewhere}!</b>`,
...
function spawnSpecialMobs(helpers, io) {
    ...
    if (gameState_1.superMobCount.value > 0 && gameState_1.uniqueMobCount.value === 0 && Math.random() < 0.25) {
        ...
        if (uniqueMob) {
            ...
            announceBossSpawn('unique', uniqueMob, getSectionAtPosition((0, mobFields_1.mobX)(uniqueMob.entity), (0, mobFields_1.mobY)(uniqueMob.entity)), io);
