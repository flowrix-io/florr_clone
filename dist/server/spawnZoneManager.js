...
exports.updateSpawnZones = updateSpawnZones;
...
const TRICKLE_INTERVAL_MS = 4000;
const TRICKLE_MIN = 1;
const TRICKLE_MAX = 2;
...
let zoneStates = null;
function computeTargetMobCount(zone) {
    ...
    const density = constants_1.ORIGINAL_ENEMY_COUNT / constants_1.TOTAL_WORLD_AREA;
    const areaWorld = (zone.width * constants_1.SCALE_FACTOR) * (zone.height * constants_1.SCALE_FACTOR);
    return Math.max(1, Math.ceil(density * areaWorld));
}
function ensureInitialized() {
    if (zoneStates !== null)
        return;
    const zones = (0, enemySpawner_1.getAllSpawnZones)();
 ...
 */
function updateSpawnZones(helpers) {
    ensureInitialized();
    ...
    const now = Date.now();
    for (const state of zoneStates) {
        const nearViewport = (0, enemySpawner_1.isZoneNearAnyViewport)(state.zone, viewports);
        if (!nearViewport) {
        ...
        if (!state.initialized) {
            ...
            const currentInZone = (0, enemySpawner_1.countMobsInZone)(state.zone);
