...
export class TitleScreen {
    ...
    private settingsShowHitboxes: boolean = false;
    private settingsShadersEnabled: boolean = false;
    private settingsShowStats: boolean = false;
    private settingsMobFramerate: number = 15;
    private settingsHighQualityMobs: boolean = false;
    private settingsDynamicSkybox: boolean = false;
    private settingsMobDeathAnimation: boolean = true;
    private settingsInterpolation: number = 0.15;
    private settingsShowConsoleLogs: boolean = false;
    private settingsServerIP: string = '';
    private settingsServerIPFocused: boolean = false;
    private settingsSliderDragging: string | null = null; // 'mobFramerate' or 'interpolation'
        ...
        this.uiCanvas.addEventListener('mousedown', (e) => {
            if (this.settingsOpen && this.settingsHoveredItem) {
                ...
                if (this.settingsHoveredItem === 'slider_mobFramerate' || this.settingsHoveredItem === 'slider_interpolation') {
                    ...
                    const { x } = canvasCoords(this.uiCanvas, e as MouseEvent);
                    this.handleSliderDrag(x);
                }
            } else if (this.hoveredStartButton) this.pressedButton = 'start';
    ...
    private renderSettingsMenu(ctx: CanvasRenderingContext2D): void {
        ...
        this.drawGardnButton(ctx, closeBtnX, closeBtnY, closeBtnSize, closeBtnSize,
    ...
    private drawSettingsCheckbox(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, checked: boolean, label: string, hovered: boolean): void {
    ...
    private drawSettingsSlider(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, ratio: number, id: string): void {
    ...
    private handleSettingsHover(x: number, y: number): void {
        this.settingsHoveredItem = null;
        const { panelW, panelH, panelX, panelY, pad, tabH, headerH, contentX, contentW, contentTop, contentBottom } = this.getSettingsLayout();
    ...
    private toggleSettingsCheckbox(id: string): void {
        switch (id) {
            case 'showHitboxes':
    ...
    private handleSliderDrag(mouseX: number): void {
