...
export function startSession(username: string, token: string, serverUrl?: string): void {
    if (!hasStorage()) return;
    try {
...
export interface LoginResult {
    /** false = server rejected the credentials (see status). Network errors throw. */
    ok: boolean;
    token?: string;
    status: number;
}
 ...
 */
export async function requestSessionToken(serverUrl: string, username: string, password: string): Promise<LoginResult> {
 ...
 */
export function migrateLegacyCredentials(): Promise<void> {
