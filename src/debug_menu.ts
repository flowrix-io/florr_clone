...
export function isDebugMenuEnabled(): boolean {
    return localStorage.getItem(DEBUG_MENU_SETTING_KEY) === 'true';
}

export interface ServerDebugStats {
    rss: number;        // bytes
    heapUsed: number;   // bytes
    heapTotal: number;  // bytes
    tickAvgMs: number;  // mean tick duration over the last second
    tickMaxMs: number;  // worst tick duration over the last second
}
...
function pushSample(series: number[], value: number): void {
    series.push(value);
...
export class DebugMenuPanel {
    ...
    private clientMemMB: number[] = [];
    private serverTickAvgMs: number[] = [];
    private serverTickMaxMs: number[] = [];
    private serverHeapMB: number[] = [];
    private serverRssMB: number[] = [];
    private lastServerStats: ServerDebugStats | null = null;
    ...
    public isMenuOpen(): boolean { return this.open; }
    ...
    public recordServerStats(stats: ServerDebugStats): void {
        if (!stats || typeof stats.heapUsed !== 'number') return;
    ...
    public render(ctx: CanvasRenderingContext2D): void {
        ...
        if (this.open && !isDebugMenuEnabled()) this.close();
        if (!this.open) return;
        ...
        drawGardnButton(ctx, closeBtnX, closeBtnY, 28, 28, '#cc4444', this.closeHovered, false, 'X', 16, 3, 3);
        ...
        const last = (s: number[]) => (s.length > 0 ? s[s.length - 1] : undefined);

        const frameMs = last(this.clientFrameMs);
        this.drawGraph(ctx, contentX, cy, contentW, graphH, 'Client Frame Time',
            frameMs !== undefined ? `${frameMs.toFixed(1)} ms (${Math.round(1000 / Math.max(frameMs, 0.01))} FPS)` : 'collecting…',
            [{ series: this.clientFrameMs, color: '#5a9fdb' }], 'ms');
        cy += labelH + graphH + gap;

        const cliMem = last(this.clientMemMB);
        this.drawGraph(ctx, contentX, cy, contentW, graphH, 'Client Memory (JS heap)',
            cliMem !== undefined ? `${cliMem.toFixed(1)} MB` : 'unavailable (Chrome only)',
            [{ series: this.clientMemMB, color: '#7fdb7f' }], 'MB');
        cy += labelH + graphH + gap;

        const noServer = this.lastServerStats === null;
        const tickAvg = last(this.serverTickAvgMs);
        const tickMax = last(this.serverTickMaxMs);
        this.drawGraph(ctx, contentX, cy, contentW, graphH, 'Server Tick Time',
            noServer ? 'no data — join a game' : `avg ${tickAvg!.toFixed(1)} / max ${tickMax!.toFixed(1)} ms`,
            [
        ...
        this.drawGraph(ctx, contentX, cy, contentW, graphH, 'Server Memory',
            noServer ? 'no data — join a game' : `heap ${srvHeap!.toFixed(1)} / rss ${srvRss!.toFixed(1)} MB`,
            [
    ...
    private drawGraph(
        ...
        lines: Array<{ series: number[]; color: string }>,
        unit: string,
    ): void {
        const labelH = 18;
        ...
        ctx.beginPath();
        drawRoundedRect(ctx, x, plotY, w, h, 3);
        ctx.fill();
...
export const debugMenuPanel = new DebugMenuPanel();
