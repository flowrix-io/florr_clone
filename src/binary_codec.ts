...
class Writer {
    ...
    private grow(min: number) {
        ...
        if (this.pos + n > this.buf.length) this.grow(this.pos + n);
    }

    u8(v: number) { this.ensure(1); this.buf[this.pos++] = v; }
    i8(v: number) { this.ensure(1); this.dv.setInt8(this.pos, v); this.pos += 1; }
    u16(v: number) { this.ensure(2); this.dv.setUint16(this.pos, v, true); this.pos += 2; }
    i16(v: number) { this.ensure(2); this.dv.setInt16(this.pos, v, true); this.pos += 2; }
    i32(v: number) { this.ensure(4); this.dv.setInt32(this.pos, v, true); this.pos += 4; }
    u32(v: number) { this.ensure(4); this.dv.setUint32(this.pos, v, true); this.pos += 4; }
    ...
    bytes(b: Uint8Array) { this.ensure(b.length); this.buf.set(b, this.pos); this.pos += b.length; }

    writeValue(v: any) {
        if (v === null) { this.u8(TAG_NULL); return; }
        if (v === undefined) { this.u8(TAG_UNDEF); return; }
        ...
        if (t === 'number') {
            ...
            if (Number.isInteger(v)) {
                if (v >= 0 && v <= 0x7F) { this.u8(v); return; }
                if (v >= -0x80 && v <= 0x7F) { this.u8(TAG_INT8); this.i8(v); return; }
                if (v >= -0x8000 && v <= 0x7FFF) { this.u8(TAG_INT16); this.i16(v); return; }
                if (v >= -0x80000000 && v <= 0x7FFFFFFF) { this.u8(TAG_INT32); this.i32(v); return; }
                if (v >= 0 && v <= 0xFFFFFFFF) { this.u8(TAG_UINT32); this.u32(v); return; }
                // Larger integers (e.g. ms timestamps) fall back to float64 — lossless up to 2^53.
                this.u8(TAG_FLOAT64); this.f64(v); return;
            }
            // Non-integer floats: float32 is enough precision for positions, angles, and speeds.
            this.u8(TAG_FLOAT32); this.f32(v); return;
        }
        ...
        if (t === 'string') {
            ...
            if (n <= 31) {
                ...
                this.u8(TAG_STR8); this.u8(n);
            } else if (n <= 0xFFFF) {
                this.u8(TAG_STR16); this.u16(n);
            } else {
        ...
        if (v instanceof Uint8Array) {
            const n = v.length;
            if (n <= 0xFF) { this.u8(TAG_BIN8); this.u8(n); }
            else if (n <= 0xFFFF) { this.u8(TAG_BIN16); this.u16(n); }
            else throw new Error('binary_codec: binary too long (' + n + ')');
            this.bytes(v);
            return;
        }
        ...
        if (Array.isArray(v)) {
            ...
            if (n <= 31) {
                ...
                this.u8(TAG_ARR16); this.u16(n);
            } else {
                throw new Error('binary_codec: array too long (' + n + ')');
            }
            for (let i = 0; i < n; i++) this.writeValue(v[i]);
            return;
        }
...
class Reader {
    ...
    private readMap(n: number) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (tag >= FIXSTR_BASE && tag <= FIXSTR_BASE + 31) {
                ...
                const len = this.dv.getUint16(this.pos, true); this.pos += 2;
                k = this.readStr(len);
            } else {
    ...
    read(): any {
        ...
        if (tag >= FIXSTR_BASE && tag <= FIXSTR_BASE + 31) return this.readStr(tag & 0x1F);
        if (tag >= FIXARR_BASE && tag <= FIXARR_BASE + 31) return this.readArr(tag & 0x1F);
        if (tag >= FIXMAP_BASE && tag <= FIXMAP_BASE + 31) return this.readMap(tag & 0x1F);
        switch (tag) {
            case TAG_NULL: return null;
            case TAG_UNDEF: return undefined;
            case TAG_FALSE: return false;
            case TAG_TRUE: return true;
            case TAG_INT8: { const v = this.dv.getInt8(this.pos); this.pos += 1; return v; }
            case TAG_INT16: { const v = this.dv.getInt16(this.pos, true); this.pos += 2; return v; }
            case TAG_INT32: { const v = this.dv.getInt32(this.pos, true); this.pos += 4; return v; }
            case TAG_UINT32: { const v = this.dv.getUint32(this.pos, true); this.pos += 4; return v; }
            case TAG_FLOAT32: { const v = this.dv.getFloat32(this.pos, true); this.pos += 4; return v; }
            case TAG_FLOAT64: { const v = this.dv.getFloat64(this.pos, true); this.pos += 8; return v; }
            case TAG_STR8: { const n = this.buf[this.pos++]; return this.readStr(n); }
            case TAG_STR16: { const n = this.dv.getUint16(this.pos, true); this.pos += 2; return this.readStr(n); }
            case TAG_ARR16: { const n = this.dv.getUint16(this.pos, true); this.pos += 2; return this.readArr(n); }
            case TAG_MAP16: { const n = this.dv.getUint16(this.pos, true); this.pos += 2; return this.readMap(n); }
            case TAG_BIN8: { const n = this.buf[this.pos++]; return this.readBytes(n); }
            case TAG_BIN16: { const n = this.dv.getUint16(this.pos, true); this.pos += 2; return this.readBytes(n); }
        }
