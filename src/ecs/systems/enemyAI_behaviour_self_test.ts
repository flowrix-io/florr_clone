...
import { spawnMob, spawnPlayer, makePet } from '../prefabs';
...
export function runEnemyAiBehaviourSelfTest(): string[] {
    ...
    interface HarnessOptions {
        losBlocked?: boolean;
        blockedTiles?: (x: number, y: number) => boolean;
    }

    function makeHarness(options: HarnessOptions = {}) {
        const world = new World();
        const scheduler = new Scheduler(world);
        const volleys: Array<{ shooter: Entity; aim: number }> = [];
        const lostPets: Entity[] = [];
        let losCalls = 0;
    ...
    function addMob(world: World, id: string, x: number, y: number, aiType: C.AiType, speed = 50, range = 500): Entity {
    ...
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'hostile', 0, 0, C.AiType.Hostile);
        const player = addPlayer(h.world, 'p1', 200, 0);

        h.tick();
        checkEqual('hostile mob acquires the player',
            h.world.get(mob, C.MobAI, 'targetPlayer'), player);
        checkEqual('mob is marked chasing', h.world.get(mob, C.MobAI, 'isChasing'), 1);
        check('mob moved toward the player', (h.world.get(mob, C.Position, 'x') as number) > 0);
    ...
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'far', 0, 0, C.AiType.Hostile, 50, 300);
        addPlayer(h.world, 'p2', 5000, 0);

        h.tick();
        checkEqual('distant player is not acquired',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);
    ...
    // -- occluded targets are not acquired -------------------------------------
    {
        ...
        const mob = addMob(h.world, 'blind', 0, 0, C.AiType.Hostile);
        addPlayer(h.world, 'p3', 100, 0);

        h.tick();
        checkEqual('occluded player is not acquired',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);
    }
    ...
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'neutral', 0, 0, C.AiType.Neutral);
        const player = addPlayer(h.world, 'p4', 100, 0);

        h.tick();
        checkEqual('unprovoked neutral does not scan',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);
        ...
        check('provoked neutral closes distance', (h.world.get(mob, C.Position, 'x') as number) > before);
    }

    // -- passive mobs never scan ------------------------------------------------
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'passive', 0, 0, C.AiType.Passive);
        addPlayer(h.world, 'p5', 50, 0);

        h.tick();
        checkEqual('passive mob acquires nothing',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);
    ...
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'sticky', 0, 0, C.AiType.Hostile);
        ...
        checkEqual('cached target is kept even with a closer player',
            h.world.get(mob, C.MobAI, 'targetPlayer'), near);
    ...
    {
        const h = makeHarness();
        const mob = addMob(h.world, 'drop', 0, 0, C.AiType.Hostile);
        const player = addPlayer(h.world, 'dying', 100, 0);
        h.world.set(mob, C.MobAI, 'targetPlayer', player);

        h.world.add(player, C.IsDead);
        h.tick();
        checkEqual('dead target is dropped',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);
    }
    ...
    // -- a destroyed target does not alias a recycled entity ---------------------
    {
        ...
        // as "target gone".
        const h = makeHarness();
        const mob = addMob(h.world, 'alias', 0, 0, C.AiType.Hostile, 0);
        ...
        const target = h.world.get(mob, C.MobAI, 'targetPlayer') as Entity;
        check('destroyed target does not alias the recycled slot', target !== replacement,
            'the mob retargeted onto whatever reused the slot');
    }

    // -- hole tether: mobs return home past the retreat radius -------------------
    {
        const h = makeHarness();
        const hole = addMob(h.world, 'hole', 0, 0, C.AiType.Passive, 0);
        const ant = addMob(h.world, 'ant', 1000, 0, C.AiType.Hostile);
        ...
        h.tick();
        checkEqual('dragged past the retreat radius, the ant gives up',
            h.world.get(ant, C.MobAI, 'targetPlayer'), NULL_ENTITY);
        checkEqual('and starts returning', h.world.get(ant, C.HoleTether, 'returning'), 1);
        check('and moves toward its hole', (h.world.get(ant, C.Position, 'x') as number) < 1000);
    }

    // -- hole tether: arriving home resumes normal AI ----------------------------
    {
        const h = makeHarness();
        const hole = addMob(h.world, 'hole2', 0, 0, C.AiType.Passive, 0);
        const ant = addMob(h.world, 'ant2', 50, 0, C.AiType.Hostile);
        h.world.add(ant, C.HoleTether, { hole, returning: 1 });
        h.world.add(ant, C.PassiveMotion, { state: C.PassiveState.Moving, stateStart: 0 });

        h.tick();
        checkEqual('within 100u the ant stops returning',
            h.world.get(ant, C.HoleTether, 'returning'), 0);
        checkEqual('and its passive machine is reset to idle',
            h.world.get(ant, C.PassiveMotion, 'state'), C.PassiveState.Idle);
    }

    // -- hole tether: a destroyed hole unparents ---------------------------------
    {
        const h = makeHarness();
        const hole = addMob(h.world, 'hole3', 0, 0, C.AiType.Passive, 0);
        const ant = addMob(h.world, 'ant3', 2000, 0, C.AiType.Hostile);
        h.world.add(ant, C.HoleTether, { hole, returning: 1 });

        h.world.destroy(hole);
        h.tick();
        check('destroyed hole unparents the mob, which roams free',
            !h.world.has(ant, C.HoleTether));
    }
    ...
    // -- pets follow their owner --------------------------------------------------
    {
        ...
        const pet = addMob(h.world, 'pet', 0, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);

        h.tick();
        check('pet moves toward its owner', (h.world.get(pet, C.Position, 'x') as number) > 0);
    }
    ...
    // -- pets teleport when sight to the owner is lost ----------------------------
    {
        ...
        const pet = addMob(h.world, 'pet2', 0, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);

        h.tick();
        // With every ring position failing the sight test too, the pet falls back
        // to the owner's own tile.
        checkEqual('pet ends up on its owner when nothing is visible',
            h.world.get(pet, C.Position, 'x'), 3000);
    }
    ...
    // -- pets attack wild mobs, not each other ------------------------------------
    {
        ...
        const pet = addMob(h.world, 'pet3', 100, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
        const otherPet = addMob(h.world, 'pet4', 120, 0, C.AiType.Hostile);
        makePet(h.world, otherPet, owner);
        const wild = addMob(h.world, 'wild', 150, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('pet targets the wild mob', h.world.get(pet, C.MobAI, 'targetEnemy'), wild);
        check('pet does not target another pet',
            (h.world.get(pet, C.MobAI, 'targetEnemy') as Entity) !== otherPet);
    }
    ...
    // -- a pet's sight is its owner's screen ---------------------------------------
    {
        ...
        const pet = addMob(h.world, 'petView', 900, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
        addMob(h.world, 'offscreen', 1300, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('a mob off the owner\'s screen is not targeted',
            h.world.get(pet, C.MobAI, 'targetEnemy'), NULL_ENTITY);
    }
    {
        // Beyond the pet's own range but on the owner's screen: targeted.
        const h = makeHarness();
        const owner = addPlayer(h.world, 'ownerView2', 0, 0);
        const pet = addMob(h.world, 'petView2', 0, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
        const wild = addMob(h.world, 'onscreen', 800, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('a mob on the owner\'s screen is targeted past the pet\'s own range',
            h.world.get(pet, C.MobAI, 'targetEnemy'), wild);
    }
    ...
    // -- passive pets never engage; neutral pets turn hostile ------------------------
    {
        ...
        const passivePet = addMob(h.world, 'petPassive', 100, 0, C.AiType.Passive);
        makePet(h.world, passivePet, owner);
        const neutralPet = addMob(h.world, 'petNeutral', 100, 50, C.AiType.Neutral);
        makePet(h.world, neutralPet, owner);
        const wild = addMob(h.world, 'wildCalm', 150, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('a passive pet takes no target',
            h.world.get(passivePet, C.MobAI, 'targetEnemy'), NULL_ENTITY);
        check('a passive pet fires no volley',
            !h.volleys.some(v => v.shooter === passivePet));
        checkEqual('a neutral pet attacks like a hostile one',
            h.world.get(neutralPet, C.MobAI, 'targetEnemy'), wild);
    }
    ...
    // -- sandstorm pets shadow the owner's movement ----------------------------------
    {
        ...
        const storm = addMob(h.world, 'petStorm', 100, 0, C.AiType.Sandstorm);
        makePet(h.world, storm, owner);

        h.tick();
        checkEqual('a sandstorm pet stands still while its owner does',
            h.world.get(storm, C.Position, 'x'), 100);

        h.world.write(owner, C.Velocity, { x: 300, y: 0 });
        h.tick();
        const x = h.world.get(storm, C.Position, 'x') as number;
        // Owner moves 300/30 = 10px per tick; the pet must beat that.
        check('a sandstorm pet moves the way its owner moves, slightly faster',
            x > 110, `expected x > 110, got ${x}`);
        checkEqual('a sandstorm pet never takes a target',
            h.world.get(storm, C.MobAI, 'targetEnemy'), NULL_ENTITY);
    }
    ...
    // -- off-screen sandstorm and passive pets despawn instead of teleporting --------
    {
        ...
        const storm = addMob(h.world, 'lostStorm', 2000, 0, C.AiType.Sandstorm);
        makePet(h.world, storm, owner);
        const calm = addMob(h.world, 'lostCalm', 0, 2000, C.AiType.Passive);
        makePet(h.world, calm, owner);
        ...
        checkEqual('the lost sandstorm pet was not teleported',
            h.world.get(storm, C.Position, 'x'), 2000);
        checkEqual('the lost passive pet was not teleported',
            h.world.get(calm, C.Position, 'y'), 2000);
    }
    {
        ...
        const calm = addMob(h.world, 'walledCalm', 0, 0, C.AiType.Passive);
        makePet(h.world, calm, owner);

        h.tick();
        checkEqual('a sight-blocked on-screen passive pet is not despawned', h.lostPets.length, 0);
        checkEqual('and holds position instead of teleporting',
            h.world.get(calm, C.Position, 'x'), 0);
    }
    {
        // Hostile pets keep the teleport recovery.
        const h = makeHarness({ losBlocked: true });
        const owner = addPlayer(h.world, 'ownerHost', 3000, 3000);
        const pet = addMob(h.world, 'hostPet', 0, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
    ...
    // -- wild mobs fall back to pets when no player is in range --------------------
    {
        ...
        const pet = addMob(h.world, 'pet5', 100, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
        const wild = addMob(h.world, 'wild2', 0, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('wild mob targets the pet when no player is near',
            h.world.get(wild, C.MobAI, 'targetPet'), pet);
    ...
    // -- players outrank pets -------------------------------------------------------
    {
        ...
        const pet = addMob(h.world, 'pet6', 100, 0, C.AiType.Hostile);
        makePet(h.world, pet, owner);
        const player = addPlayer(h.world, 'target', 150, 0);
        const wild = addMob(h.world, 'wild3', 0, 0, C.AiType.Hostile);

        h.tick();
        checkEqual('the player is preferred', h.world.get(wild, C.MobAI, 'targetPlayer'), player);
        checkEqual('and no pet target is taken',
            h.world.get(wild, C.MobAI, 'targetPet'), NULL_ENTITY);
    }
    ...
    // -- aggro bonus widens detection ------------------------------------------------
    {
        ...
        // many pixels closer.
        const h = makeHarness();
        const mob = addMob(h.world, 'aggro', 0, 0, C.AiType.Hostile, 50, 100);
        const player = addPlayer(h.world, 'bulb', 300, 0);

        h.tick();
        checkEqual('outside plain range, nothing is acquired',
            h.world.get(mob, C.MobAI, 'targetPlayer'), NULL_ENTITY);

        h.world.set(player, C.PlayerModifiers, 'aggroRadiusBonus', 400);
        h.tick();
        checkEqual('the aggro bonus brings the player into range',
            h.world.get(mob, C.MobAI, 'targetPlayer'), player);
    }
    ...
    {
        const h = makeHarness();
        const storm = spawnMob(h.world, {
            id: 'storm', type: 'sandstorm', tier: 'super', x: 0, y: 0,
            health: 100, maxHealth: 100, speed: 50, damage: 5, radius: 40,
            aiType: C.AiType.Sandstorm, now: 0,
        });
        const player = addPlayer(h.world, 'sucked', 200, 0);

        h.tick();
        check('a super sandstorm pulls the player toward it',
            (h.world.get(player, C.Position, 'x') as number) < 200);
    ...
    {
        const h = makeHarness();
        const shooter = addMob(h.world, 'shooter', 0, 0, C.AiType.Hostile);
        ...
        check('aim uses the pre-move offset',
            Math.abs(h.volleys[0].aim - Math.PI / 2) < 1e-9,
            `aim was ${h.volleys[0].aim}`);
    }

    // -- lobby and dead players are invisible to AI -----------------------------------------
    {
        const h = makeHarness();
        const picky = addMob(h.world, 'picky', 0, 0, C.AiType.Hostile);
        ...
        h.tick();
        const target = h.world.get(picky, C.MobAI, 'targetPlayer') as Entity;
        check('a title-screen player is never targeted', target !== lobby);
        check('a dead player is never targeted', target !== dead);
        checkEqual('nothing was acquired', target, NULL_ENTITY);
    }

    // -- centipede body segments skip AI -----------------------------------------------------
    {
        const h = makeHarness();
        const head = addMob(h.world, 'chead', 0, 0, C.AiType.Hostile);
        h.world.add(head, C.CentipedeSegment, { leader: NULL_ENTITY, head, segmentIndex: 0 });
        const body = addMob(h.world, 'cbody', 100, 0, C.AiType.Hostile);
        ...
        checkEqual('body segment does not target', h.world.get(body, C.MobAI, 'targetPlayer'), NULL_ENTITY);
        checkEqual('head does target', h.world.get(head, C.MobAI, 'targetPlayer') !== NULL_ENTITY, true);
    }
