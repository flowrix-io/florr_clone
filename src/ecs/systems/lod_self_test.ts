...
export function runLodSelfTest(): Failure[] {
    ...
    {
        const field = new MobActivityField();
        check('empty field treats everything as active', field.isActive(1e6, -1e6));
        check('empty field steps every mob every tick',
            field.shouldStep(makeEntity(7, 1), 1e6, -1e6, 1));

        // ...and stops being permissive the moment a player exists, or the
        // whole mechanism would be a no-op in production.
        field.add(0, 0);
        check('a populated field is no longer permissive',
            !field.isActive(MOB_ACTIVE_RADIUS * 3, 0));
    }
    ...
    // -- the radius is measured as a radius, not a box ------------------------
    {
        ...
        check('mob just inside the radius is active',
            field.isActive(MOB_ACTIVE_RADIUS - 1, 0));
        check('mob just outside the radius is not',
            !field.isActive(MOB_ACTIVE_RADIUS + 1, 0));
        ...
        check('diagonal distance is euclidean, not per-axis',
            !field.isActive(MOB_ACTIVE_RADIUS * 0.8, MOB_ACTIVE_RADIUS * 0.8));
    }
    ...
    // -- any player counts, not just the first --------------------------------
    {
        const field = new MobActivityField();
        field.add(0, 0);
        field.add(50_000, 50_000);
        check('a mob near the SECOND player is active', field.isActive(50_000, 50_000));
        check('a mob near neither is not', !field.isActive(25_000, 25_000));

        field.clear();
        check('clear() restores the permissive default', field.isActive(25_000, 25_000));
    }
    ...
    // -- distant mobs are strided, and spread across the stride ---------------
    {
        const field = new MobActivityField();
        field.add(0, 0);
        const far = MOB_ACTIVE_RADIUS * 10;

        // One mob: steps exactly once per stride window.
        const mob = makeEntity(3, 1);
        let stepped = 0;
        for (let tick = 0; tick < MOB_FAR_STRIDE * 20; tick++) {
            if (field.shouldStep(mob, far, 0, tick)) stepped++;
        }
        ...
        let nearSteps = 0;
        for (let tick = 0; tick < MOB_FAR_STRIDE * 20; tick++) {
            if (field.shouldStep(mob, 10, 10, tick)) nearSteps++;
        }
        checkEqual('a near mob steps every tick', nearSteps, MOB_FAR_STRIDE * 20);
        ...
        const population = MOB_FAR_STRIDE * 40;
        for (let slot = 0; slot < population; slot++) {
            if (field.shouldStep(makeEntity(slot, 1), far, 0, 0)) steppedThisTick++;
        }
        checkEqual('distant work is spread evenly over the stride',
            steppedThisTick, population / MOB_FAR_STRIDE);
    }
    ...
    // -- the refresh system reads live player positions -----------------------
    {
        ...
        const field = new MobActivityField();
        registerMobActivitySystem(scheduler, field, createMobActivityQueries(world));
        ...
        world.write(player, C.Position, { x: 400_000, y: 400_000 });
        scheduler.tick(1 / 30, 1000 / 30, 1033);
        check('the field follows the player', field.isActive(400_100, 400_000));
