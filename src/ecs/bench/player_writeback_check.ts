 ...
 */
function pvpKnockback(
    attacker: ServerPlayer,
    victim: ServerPlayer,
    apply: (victim: ServerPlayer, dx: number, dy: number) => void,
): { x: number; y: number } {
...
function displaceXYOnly(player: ServerPlayer, dx: number, dy: number): void {
    player.x += dx;
    player.y += dy;
}
 ...
 */
function measurePvpKnockback(
    attackerFirst: boolean,
    apply: (victim: ServerPlayer, dx: number, dy: number) => void,
): { appliedX: number; expectedX: number } {
    resetSyncState();
    ...
    const players: Record<string, ServerPlayer> = { attacker, victim, twin };
    ...
    for (let tick = 0; tick < 6; tick++) {
        runPlayerPhase(runtime, players, order, tick, player => {
            if (tick !== 3 || player.id !== 'attacker') return;
 ...
 */
function measureRevivePosition(reviverFirst: boolean): { landedX: number; corpseX: number } {
    resetSyncState();
    const runtime = makeRuntime();
    configureCutover(runtime);
    ...
    const players: Record<string, ServerPlayer> = { reviver, corpse };
    ...
    for (let tick = 0; tick < 12; tick++) {
        runPlayerPhase(runtime, players, order, tick, player => {
            if (tick !== REVIVE_TICK || player.id !== 'reviver') return;
        ...
        if (tick === DEATH_TICK) corpse.isDead = true;
        if (tick === DRAG_TICK) corpse.x = CORPSE_DESTINATION;
    }
...
export function runPlayerWritebackCheck(): string[] {
    ...
    for (const attackerFirst of [true, false]) {
        ...
        const { appliedX, expectedX } = measurePvpKnockback(attackerFirst, displacePlayer);
