...
import { EntityCursor, World } from './world';
...
export function runEcsSelfTest(): Failure[] {
    ...
    // -- entity cursors ------------------------------------------------------
    {
        ...
        checkEqual('cursor reads the same value as get',
            cursor.need(Position).x[cursor.row], world.get(e, Position, 'x'));
        ...
        let threw = false;
        try { cursor.need(Velocity); } catch { threw = true; }
        ...
        checkEqual('re-seek reads the added component',
            cursor.need(Velocity).x[cursor.row], 1);
        ...
        const stale = makeEntity(entityIndex(e), entityGeneration(e) + 1);
        checkEqual('seek fails for a stale generation', world.seek(stale, cursor), false);
        checkEqual('seek fails for NULL_ENTITY', world.seek(NULL_ENTITY, cursor), false);
    }
