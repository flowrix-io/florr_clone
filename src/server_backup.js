...
io.on('connection', (socket) => {
    ...
    socket.on('playerMovement', (movementData) => {
        ...
        if (player) {
            ...
            // Debug: Log player movement if significant
            const moveDistance = Math.sqrt((newX - player.x) ** 2 + (newY - player.y) ** 2);
            if (moveDistance > 10) { // Only log significant movements to avoid spam
                console.log(`[SERVER] Player ${socket.id} movement: (${player.x.toFixed(1)}, ${player.y.toFixed(1)}) -> (${newX.toFixed(1)}, ${newY.toFixed(1)}) distance:${moveDistance.toFixed(1)}`);
            }
            ...
            if (!collision) {
                ...
                if (Math.abs(newX - player.x) > 5 || Math.abs(newY - player.y) > 5) {
                    console.log(`[SERVER] Player position updated: (${player.x.toFixed(1)}, ${player.y.toFixed(1)}) -> (${newX.toFixed(1)}, ${newY.toFixed(1)})`);
                }
                player.x = newX;
                player.y = newY;
