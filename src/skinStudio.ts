...
export class SkinStudio {
    ...
    constructor() {
        this.keyHandler = (e: KeyboardEvent) => {
            ...
            if (e.key === 'Escape' && !this.nameInput) this.hide();
        };
    ...
    public hide(): void { this.isOpen_ = false; this.closeNameInput(); this.drag = null; }
    ...
    private dispatch(a: Action): void {
        switch (a.k) {
            ...
            case 'reset': this.shapes = starterShapes(); this.selected = 0; break;
    ...
    private drawShapeList(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number): void {
        ...
        this.shapes.forEach((s, i) => {
            if (ry + rowH > y && ry < y + h) {
                ...
                ctx.fillStyle = sel ? 'rgba(39,218,222,0.18)' : ROW_BG; ctx.fill();
    ...
    private button(
        ...
        bg = ACCENT, border = BORDER, fg = '#06343a', font = '12px', align: 'center' | 'left' = 'center',
    ): void {
    ...
    private iconBtn(ctx: CanvasRenderingContext2D, x: number, y: number, kind: 'up' | 'down' | 'del', action: Action): void {
        ...
        roundRect(ctx, x + 1, y + 1, s - 2, s - 2, 2); ctx.fillStyle = hovered ? '#3a4046' : ROW_BG; ctx.fill();
