...
export class InventoryManager {
            ...
            for (const rarity of RARITY_LEVELS) {
                ...
                if (hasKilled && mobRarities.includes(rarity)) {
                    ...
                    if (mobStats) {
                        ...
                        if (mobStats.image) {
                            this.convertSVGToBitmap(mobStats.image, 32, 32).then((dataUrl: string | null) => {
                                if (dataUrl) {
                                    mobIcon.src = dataUrl;
                                }
                            }).catch((error: Error) => {
    ...
    private async convertSVGToBitmap(svgString: string, width: number, height: number): Promise<string | null> {
        try {
            // Create canvas
    ...
    private createDropItemElement(itemName: string, finalRarity: string, finalProb: string, itemType: string, dropType: string): HTMLElement {
    ...
    private showMobTooltip(element: HTMLElement, mobType: string, rarity: string): void {
        ...
        if (dropTable) {
            ...
            const upgradeRarity = (rarity: Rarity): Rarity => {
            ...
            const downgradeRarity = (rarity: Rarity): Rarity => {
                const currentIndex = RARITY_ORDER.indexOf(rarity);
            ...
            for (const drop of processedDrops) {
                ...
                if (!isCommon && rarityIndex > 0 && rarityIndex < RARITY_ORDER.length) {
                    ...
                    const lowerRarity = RARITY_ORDER[rarityIndex - 1] as Rarity;
