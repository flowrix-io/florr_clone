...
type ButtonKind =
    ...
    | { type: 'acceptInvite' }
    | { type: 'declineInvite' }
    | { type: 'kick'; member: string }
    | { type: 'squadOne'; member: string };
 ...
 * Accent color: #27dade.
 */
export class GuildMenuManager {
    private canvas: HTMLCanvasElement | null = null;
    private ctx: CanvasRenderingContext2D | null = null;
    private socket: Socket | null = null;
    private isOpen: boolean = false;

    private currentGuild: GuildState | null = null;
    ...
    private readonly HEADER_HEIGHT = 54;
    private readonly BORDER_W = 4;
    private readonly ROW_HEIGHT = 34;

    private readonly ACCENT = '#27dade';
    private readonly BORDER = '#1fb3b0';
    private readonly CLOSE_BG = '#dc7e92';
    private readonly CLOSE_BORDER = '#b56476';
    ...
    private currentUsername(): string {
        return (this.socket as any)?.username || '';
    }
    ...
    private regionKey(region: HitRegion | null): string | null {
        if (!region) return null;
        if (region.kind.type === 'kick') return `kick:${region.kind.member}`;
        if (region.kind.type === 'squadOne') return `squadOne:${region.kind.member}`;
    ...
    private handleWheel(e: WheelEvent): void {
        ...
        const y = e.clientY - rect.top;
        const off = this.panelOffset();
        if (x < off.x || x > off.x + this.PANEL_WIDTH || y < off.y || y > off.y + this.PANEL_HEIGHT) return;
        e.preventDefault();
        const visible = this.PANEL_HEIGHT - this.HEADER_HEIGHT;
        const maxScroll = Math.max(0, this.contentHeight - visible);
    ...
    private handleAction(kind: ButtonKind): void {
        if (!this.socket) {
        ...
        switch (kind.type) {
            ...
            case 'invite':
                ...
                break;
            case 'leave':
                if (confirm('Leave this guild?')) this.socket.emit('guildLeave');
                break;
            case 'squadAll':
            ...
            case 'declineInvite':
                ...
                break;
            case 'kick':
                if (confirm(`Kick ${kind.member} from the guild?`)) this.socket.emit('guildKick', kind.member);
                break;
            case 'squadOne':
    ...
    private submitPrompt(value: string): void {
        if (!value || !this.socket) { this.closePrompt(); return; }
        switch (this.promptMode) {
            case 'create': this.socket.emit('guildCreate', value); break;
            case 'invite': this.socket.emit('guildInvite', value); break;
            case 'kick': this.socket.emit('guildKick', value); break;
        }
        this.closePrompt();
    }
    ...
    public render(): void {
        ...
        if (!this.currentGuild) {
            ...
            const sorted = guild.memberUsernames.slice().sort((a, b) => {
                ...
                if (a.toLowerCase() === guild.leaderUsername.toLowerCase()) return -1;
                if (b.toLowerCase() === guild.leaderUsername.toLowerCase()) return 1;
    ...
    private drawButton(
        ...
        label: string, labelColor: string, borderColor: string, hovered: boolean,
    ): void {
        ctx.save();
