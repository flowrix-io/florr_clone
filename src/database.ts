...
const readDatabase = () => {
    try {
        ...
        console.error(
            `[DATABASE] FATAL: ${inventoryPath} exists but could not be loaded. ` +
...
const scheduleWrite = () => {
    if (writeTimer || writeInFlight || !dbDirty) return;
...
process.on('exit', () => {
    if (loadFailed) return;
    if (!dbDirty && !writeInFlight && !writeTimer) return;
    try {
        const json = JSON.stringify(db);
        fs.writeFileSync(tmpPath, json);
        if (fs.statSync(tmpPath).size !== Buffer.byteLength(json)) return;
...
export const database = {
    ...
    removeInvalidEggs: (invalidEggTypes: Set<string>): number => {
        ...
        for (const username in db.users) {
            const user = db.users[username];
            if (user?.admin === true) adminUserIds.add(user.id);
        }
    ...
    backupDatabase: (label: string = 'manual'): { file: string; bytes: number } => {
    ...
    listDatabaseBackups: (): Array<{ file: string; bytes: number; mtimeMs: number }> => {
