...
import { appShell, AppScene } from '../app_shell';
import { getCurrentGame } from '../app_refs';
import { getPreconnectedSocket, requestPreconnect } from '../net/preconnect';
import { isLoggedIn, requestSessionToken, startSession } from '../auth_session';

export { injectTitleScreenStyles, titleScreenStyles } from './styles';

export class TitleScreen implements AppScene {
    ...
    private get logicalW(): number { return this.background.getCanvas().width / getBaseDeviceScale(); }
    ...
    constructor() {
        ...
        applyZoomCompensation(titleCanvas, true, true);
    ...
    public async appendToBody(): Promise<void> {
        ...
        appShell.attachCanvas(this.background.getCanvas());
        appShell.registerScene('title', this);
        appShell.start();
        ...
        setTimeout(() => {
            if (this.authForm.isConnectingState() && !isLoggedIn()) {
    ...
    private setupCanvasUIListeners(): void {
        const at = (e: MouseEvent) => canvasCoords(this.uiCanvas, e, true);
        const consume = (e: Event) => { e.stopImmediatePropagation(); e.preventDefault(); };
        const titleMode = () => !appShell.isGameMode();
        ...
        this.uiCanvas.addEventListener('click', (e) => {
            const { x, y } = at(e);
            if (this.canvasButtons.isPointOnButton(x, y)) { consume(e); return; }
            if (this.settings.isMenuOpen() && this.settings.handleClick(x, y)) { consume(e); return; }
            if (titleMode()) this.handleCanvasClick(x, y);
        });
        ...
        this.uiCanvas.addEventListener('mousemove', (e) => {
            ...
            if (this.settings.isMenuOpen()) this.settings.handleHover(x, y);
            if (titleMode()) this.handleCanvasHover(x, y);
        });
        ...
        this.uiCanvas.addEventListener('mouseup', (e) => {
            const { x, y } = at(e);
            if (this.canvasButtons.releaseClick(x, y)) consume(e);
        });
        ...
        const atTouch = (t: Touch) => canvasCoords(this.uiCanvas, t, true);
        this.uiCanvas.addEventListener('touchstart', (e: TouchEvent) => {
            const { x, y } = atTouch(e.changedTouches[0]);
            if (this.canvasButtons.press(x, y)) consume(e);
        }, { passive: false });
        this.uiCanvas.addEventListener('touchend', (e: TouchEvent) => {
            const { x, y } = atTouch(e.changedTouches[0]);
            if (this.canvasButtons.releaseClick(x, y)) consume(e);
        }, { passive: false });
        ...
        window.addEventListener('resize', () => {
            if (appShell.isGameMode()) return;
            applyZoomCompensation(this.uiCanvas, true, true);
        });
    }
    ...
    private renderCanvasUIInner(ctx: CanvasRenderingContext2D): void {
        ...
        if (!this.authForm.isVisible()) {
            drawText(ctx, titleText, centerX, centerY - 200,
                { size: 48, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 6 });
            ctx.restore();
        }
        else {
            drawText(ctx, titleText, centerX, centerY - 400,
                { size: 48, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 6 });
            ctx.restore();
        }
        ...
        drawText(ctx, displayName, textX, textY,
            { size: 18, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 3 });
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        drawText(ctx, 'Spawn Biome:', centerX, centerY - 50,
            { size: 18, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 4 });
        ...
        controlsText.forEach((text, index) => {
            ...
            // Black outline + white fill
            drawText(ctx, text, centerX, y,
                { size: 14, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 3 });
        });
    ...
    private renderStatsCounters(ctx: CanvasRenderingContext2D, width: number, height: number): void {
        ...
        for (const line of lines) {
            drawText(ctx, line.text, x, y,
                { size: 11, weight: 'bold', fill: line.color, stroke: '#000000', strokeWidth: 2 });
            y -= lineHeight;
        }
    ...
    private renderConnecting(ctx: CanvasRenderingContext2D, centerX: number, centerY: number): void {
        ...
        drawText(ctx, connectingText, centerX, centerY,
            { size: 24, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 4 });
    ...
    private async handleAuthLogin(): Promise<void> {
        ...
        try {
            const result = await requestSessionToken(serverUrl, username, password);
            ...
            if (result.ok) {
                ...
                startSession(username, result.token, serverUrl);
    ...
    private async handleAuthGuest(): Promise<void> {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const login = await requestSessionToken(serverUrl, guestUsername, guestPassword);
                ...
                startSession(guestUsername, login.token, serverUrl);
                sessionStorage.removeItem('isOffline');
                this.hideAuthContainer();
                if (!getPreconnectedSocket()) {
                    requestPreconnect();
