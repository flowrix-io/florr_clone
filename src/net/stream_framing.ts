...
export class FrameReader {
    ...
    constructor(maxFrameBytes: number = MAX_FRAME_BYTES) {
        this.maxFrameBytes = Math.min(maxFrameBytes, MAX_FRAME_BYTES);
        this.maxChunkBytes = Math.max(MAX_CHUNK_BYTES, this.maxFrameBytes * 2);
    }
     ...
     */
    push(chunk: Uint8Array, onFrame: (payload: Uint8Array) => void): void {
        ...
        for (;;) {
            ...
            if (available - FRAME_HEADER_BYTES < len) break;

            const from = s + FRAME_HEADER_BYTES;
            this.start = from + len;
        ...
        if (residual > this.maxFrameBytes + FRAME_HEADER_BYTES) {
