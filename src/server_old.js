...
var server_utils_1 = require("./server_utils");
...
app.use(function (req, res, next) {
    var origin = req.headers.origin || 'https://localhost:8080';
...
app.post('/auth/register', function (req, res) {
    ...
    var user = database_1.database.createUser(username, password);
    if (user) {
...
app.post('/auth/login', function (req, res) {
    ...
    var user = database_1.database.getUser(username, password);
    if (user) {
...
function createEnemy() {
    ...
    var x = Math.random() * constants_1.WORLD_WIDTH;
    // Determine tier based on x position
    var tier = 'common';
    for (var _i = 0, _a = Object.entries(constants_1.ZONE_BOUNDARIES); _i < _a.length; _i++) {
    ...
    var tierData = constants_1.ENEMY_TIERS[tier];
    return {
...
function moveEnemies() {
    constants_1.enemies.forEach(function (enemy) {
        ...
        var playerArray = Object.values(constants_1.players);
        ...
        if (enemy.type === 'octopus') {
            ...
            if (nearestPlayer) {
                if (nearestDistance < constants_1.FISH_DETECTION_RADIUS) {
                    ...
                    var dx = nearestPlayer.x - enemy.x;
                    var dy = nearestPlayer.y - enemy.y;
                    ...
                    var tierSpeedMultiplier = constants_1.ENEMY_TIERS[enemy.tier].speed;
                    var chaseSpeed = playerSpeed * tierSpeedMultiplier;
                    ...
                    var normalSpeed = constants_1.ENEMY_TIERS[enemy.tier].speed * 2;
        ...
        var zone = constants_1.ZONE_BOUNDARIES[enemy.tier];
...
function createObstacle() {
    var isEnemy = Math.random() < constants_1.ENEMY_CORAL_PROBABILITY;
    return {
        id: Math.random().toString(36).substr(2, 9),
...
function createItem() {
    var zoneIndex = Math.floor(Math.random() * 6);
    var pos = (0, server_utils_1.getRandomPositionInZone)(zoneIndex);
...
for (var i = 0; i < constants_1.OBSTACLE_COUNT; i++) {
    constants_1.obstacles.push(createObstacle());
}
// Initialize decorations
for (var i = 0; i < constants_1.DECORATION_COUNT; i++) {
    decorations.push((0, server_utils_1.createDecoration)());
}
// Initialize sands
for (var i = 0; i < constants_1.SAND_COUNT; i++) {
...
io.on('connection', function (socket) {
    ...
    socket.on('authenticate', function (credentials) { return __awaiter(void 0, void 0, void 0, function () {
        var user, savedProgress;
        return __generator(this, function (_a) {
            user = database_1.database.getUser(credentials.username, credentials.password);
            if (user) {
                socket.userId = user.id;
                socket.username = user.username;
                console.log('User authenticated, loading saved progress for userId:', user.id);
                savedProgress = database_1.database.getPlayerByUserId(user.id);
    ...
    socket.on('playerMovement', function (movementData) {
        var player = constants_1.players[socket.id];
        if (player) {
            var newX = movementData.x;
            var newY = movementData.y;
            ...
            var ITEM_PICKUP_RADIUS = 40; // Radius for item pickup
            for (var i = constants_1.items.length - 1; i >= 0; i--) {
                var item = constants_1.items[i]; // Cast to ItemWithRarity
                var dx = newX - item.x;
                var dy = newY - item.y;
                var distance = Math.sqrt(dx * dx + dy * dy);
                if (distance < ITEM_PICKUP_RADIUS) {
                    // Create a copy of the current inventory
                    var newInventory = __spreadArray([], player.inventory, true);
            ...
            var _loop_1 = function (enemy) {
                var enemySize = constants_1.ENEMY_SIZE * constants_1.ENEMY_SIZE_MULTIPLIERS[enemy.tier];
                if (newX < enemy.x + enemySize &&
                    ...
                    if (!player.isInvulnerable) {
                        ...
                        // Check if enemy dies
                        if (enemy.health <= 0) {
                            var index = constants_1.enemies.findIndex(function (e) { return e.id === enemy.id; });
                            if (index !== -1) {
                                // Award XP before removing the enemy
                                var xpGained = getXPFromEnemy(enemy);
                                addXPToPlayer(player, xpGained);
                                // Check for item drop
                                var dropChance = constants_1.DROP_CHANCES[enemy.tier];
            ...
            for (var _i = 0, enemies_1 = constants_1.enemies; _i < enemies_1.length; _i++) {
    ...
    socket.on('useItem', function (itemId) {
        ...
        var loadoutSlot = player.loadout.findIndex(function (item) { return (item === null || item === void 0 ? void 0 : item.id) === itemId; });
        ...
        var item = player.loadout[loadoutSlot]; // Cast to ItemWithRarity
        if (!item) {
        ...
        var multiplier = item.rarity ? rarityMultipliers[item.rarity] : 1;
        switch (item.type) {
            case 'health_potion':
        ...
        var cooldownTime = 10000; // 10 seconds base cooldown
    ...
    socket.on('updateLoadout', function (data) {
        var player = constants_1.players[socket.id];
        if (player) {
    ...
    var chatHistory = [];
    var MAX_CHAT_HISTORY = 100; // Keep last 100 messages
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('craftItems', function (data) {
            var player = constants_1.players[socket.id];
            if (!player)
                return;
            // Verify all items exist in player's inventory
            var validItems = data.items.every(function (craftItem) {
                return player.inventory.some(function (invItem) { return invItem.id === craftItem.id; });
            });
            if (!validItems) {
            ...
            var validCraft = data.items.every(function (item) {
            ...
            var currentRarity = firstItem.rarity || 'common';
            ...
            player.inventory = player.inventory.filter(function (invItem) {
                return !data.items.some(function (craftItem) { return craftItem.id === invItem.id; });
            });
...
var HEALTH_REGEN_RATE = 5; // Health points recovered per tick
var HEALTH_REGEN_INTERVAL = 1000; // Milliseconds between health regeneration ticks
var HEALTH_REGEN_COMBAT_DELAY = 0; // Delay before health starts regenerating after taking damage
// Add health regeneration interval
setInterval(function () {
    Object.values(constants_1.players).forEach(function (player) {
...
function savePlayerProgress(player, userId) {
    if (userId) {
        ...
        var saveResult = database_1.database.savePlayer(player.id, userId, {
            level: player.level,
            xp: player.xp,
            maxHealth: player.maxHealth,
            damage: player.damage,
...
// Add periodic saving
var SAVE_INTERVAL = 60000; // Save every minute
setInterval(function () {
    Object.entries(constants_1.players).forEach(function (_a) {
        var socketId = _a[0], player = _a[1];
...
app.post('/admin/save-progress', function (req, res) {
    var playerId = req.body.playerId;
    if (!playerId) {
        return res.status(400).json({ message: 'Player ID is required' });
    }
    var player = constants_1.players[playerId];
    var socket = io.sockets.sockets.get(playerId);
    ...
    if (command.startsWith('save')) {
        ...
        if (parts.length === 2) {
            ...
            if (player && (socket === null || socket === void 0 ? void 0 : socket.userId)) {
                savePlayerProgress(player, socket.userId);
                socket.emit('savePlayerProgress', player);
                console.log("Progress saved for player ".concat(playerId));
            }
            else {
                console.log("Player ".concat(playerId, " not found or not authenticated"));
            }
        }
            ...
            console.log("Saved progress for ".concat(savedCount_1, " players"));
        }
    }
    else if (command === 'list-players') {
        Object.entries(constants_1.players).forEach(function (_a) {
            var socketId = _a[0], player = _a[1];
            console.log("Player ID: ".concat(socketId, ", Name: ").concat(player.name, ", Level: ").concat(player.level));
        });
    }
        ...
        var newCount = parseInt(command.split(' ')[1]);
        if (!isNaN(newCount) && newCount >= 0) {
            ENEMY_COUNT = newCount;
            console.log("Max enemies set to ".concat(ENEMY_COUNT));
            adjustEnemyCount();
        }
        else {
