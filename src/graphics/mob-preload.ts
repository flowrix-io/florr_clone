import { Graphics, getAllMobTypes, getMobStats, getMobTypesBySection } from './core';
 ...
 * Kept for API compatibility.
 */
Graphics.prototype.calculateFramesPerCycle = function(this: Graphics, _svg: string, _frameTime: number): number {
    return 30;
};
...
Graphics.prototype.preloadMobFrames = function(
    ...
    mobStats: any,
    _cacheKey: string,
    _preloadPromises: Promise<void>[],
    _baseCacheKey?: string
): void {
 ...
 */
Graphics.prototype.loadMobFrames = function(this: Graphics, mobStats: any, _cacheKey: string): void {
    if (mobStats && mobStats.image) {
        this.svgRenderer.compileSVG(mobStats.image);
    }
};
