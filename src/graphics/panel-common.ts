...
/** The slice of the game the panels reach through to render item icons. */
export interface PanelGameAPI {
    getLocalPlayer(): any;
 ...
 */
export function syncCanvasSize(canvas: HTMLCanvasElement): { dpr: number; cssW: number; cssH: number } {
...
export function findItemIndex(rects: ItemRect[], rarity: string, itemType: string): number {
    for (let i = 0; i < rects.length; i++) {
        if (rects[i].rarity === rarity && rects[i].itemType === itemType) return i;
    }
    return -1;
}
...
export function drawItemSlot(
    ...
    game: PanelGameAPI,
    imgCache: Map<string, HTMLImageElement | null>,
    isItemDisabled: ((rarity: string, itemType: string) => boolean) | null,
): void {
