...
export class CanvasSkillsPanel {
    ...
    private drawHeader(ctx: CanvasRenderingContext2D, cssW: number) {
        ...
        ctx.lineJoin = 'round';
        drawText(ctx, 'Talents', cssW / 2, 14, { size: 22, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 4 });
        ctx.restore();
        ...
        drawText(ctx, tpText, bx + badgeSize / 2, by + badgeSize / 2 + 1, { size: 16, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 3 });

        // "TP" label to the right of the badge.
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        drawText(ctx, 'TP', bx + badgeSize + 6, by + badgeSize / 2 + 1, { size: 16, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 3 });
        ctx.restore();
    ...
    private drawNode(ctx: CanvasRenderingContext2D, node: NodeRect, hovered: boolean) {
        ...
        drawText(ctx, String(cost), lx, ly, { font: `bold ${fontSize.toFixed(1)}px Ubuntu, sans-serif`, fill: '#ff5050', stroke: '#000000', strokeWidth: 3 * node.scale });
        ctx.restore();
    }
    ...
    private drawStatsAndReset(ctx: CanvasRenderingContext2D, cssW: number, cssH: number) {
        ...
        drawText(ctx, fhText, rightX, baseY, { size: 13, weight: 'bold', fill: '#7eef6d', stroke: '#000000', strokeWidth: 3 });

        const bdText = `Body Damage: ${this.statBodyDamage}`;
        drawText(ctx, bdText, rightX, baseY + 16, { size: 13, weight: 'bold', fill: '#e0e0e0', stroke: '#000000', strokeWidth: 3 });
        ctx.restore();
        ...
        ctx.textBaseline = 'middle';
        ctx.lineJoin = 'round';
        drawText(ctx, 'Reset', rb.x + rb.w / 2, rb.y + rb.h / 2 + 1, { size: 14, weight: 'bold', fill: '#ffffff', stroke: '#000000', strokeWidth: 3 });
        ctx.restore();
    }
    ...
    private drawTooltip(ctx: CanvasRenderingContext2D, node: NodeRect, cssW: number, _cssH: number) {
        ...
        const { w: boxW, h: boxH } = measureTooltip(ctx, lines);
