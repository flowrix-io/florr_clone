...
const RAINDROP_RARITY_INDEX: Record<string, number> = {
 ...
 */
function hasLivePetalPosition(player: Player, loadoutIndex: number): boolean {
    const positions: any[] | undefined = (player as any).petalPositions;
    if (!positions || positions.length === 0) return false;
    for (let j = 0; j < positions.length; j++) {
        if (positions[j].loadoutIndex === loadoutIndex) return true;
    }
    return false;
}

function getRaindropAuraRadiusFromLoadout(player: Player): number {
    if (!player.loadout) return 0;
    let best = 0;
    for (let i = 0; i < player.loadout.length && i < 10; i++) {
        const item: any = player.loadout[i];
        if (!item || item.type !== 'petal' || item.petalType !== 'raindrop' || !item.rarity) continue;
        ...
        if (item.onCooldown && !hasLivePetalPosition(player, i)) continue;
...
// Per-player trail state. Tiles (grass/flowers) and dots are spawned in world
// coordinates as the player moves and persist for a fixed lifetime, so they
// stay anchored to the ground instead of following the player.
interface RaindropTile {
    x: number;
    y: number;
    spawnedAt: number;
    kind: 0 | 1; // 0 = grass blade, 1 = flower
    seedA: number; // size/lean variation
    seedB: number; // angle variation
    colorIndex: number; // stable index into FLOWER_COLORS, chosen at spawn time
}
interface RaindropDot {
    x: number;
    y: number;
    spawnedAt: number;
}
interface RaindropTrail {
    tiles: RaindropTile[];
    dots: RaindropDot[];
    lastTileSpawn: number;
    lastDotSpawn: number;
}
const raindropTrails: Map<string, RaindropTrail> = new Map();
    ...
    for (const [playerId, trail] of raindropTrails) {
        ...
        for (let i = 0; i < trail.tiles.length; i++) {
            ...
            if (tile.x + 8 < viewLeft || tile.x - 8 > viewRight ||
                tile.y + 8 < viewTop || tile.y - 8 > viewBottom) continue;
        ...
        for (let i = 0; i < trail.dots.length; i++) {
            ...
            if (dot.x + 6 < viewLeft || dot.x - 6 > viewRight ||
                dot.y + 12 < viewTop || dot.y - 6 > viewBottom) continue;
