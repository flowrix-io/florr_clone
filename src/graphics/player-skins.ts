...
import type { FlowerRenderAttributes } from './types';
import { PlayerRenderFlags } from '../player';
import { CustomSkin, SkinShape } from '../skin_format';

// Custom player skins.
//
// A skin replaces the default drawFlower() body for a player whose
// `renderFlags` has the skin's bit set (see drawPlayer in player-drawing.ts).
// The render function is invoked inside the player's translated canvas context
// (origin at the flower centre, no rotation), exactly where drawFlower() would
// otherwise run, so skins draw in the same local coordinate space: the default
// flower has radius ~25 in this space and `attrs.radius` carries the player's
// size-scaled radius. Skins should honour eyeX/eyeY so the face still tracks the
// look direction, and should leave the petal layer (drawn afterwards) untouched.

export interface PlayerSkinRenderAttributes extends FlowerRenderAttributes {
    // The full render-flag bitmask, in case a skin wants to vary on a second bit
    // (e.g. a holiday variant) rather than its own activation bit alone.
    renderFlags: number;
}

export interface PlayerSkin {
    // Activation bit. The skin renders only when this bit is set in renderFlags.
    flag: PlayerRenderFlags;
    // Human-readable name (used by the /forcelocalplayerflags render preview and
    // the set_skin admin command).
    name: string;
    // Custom body renderer. Draw in player-local space; the caller wraps the call
    // in ctx.save()/ctx.restore() so transform/state changes are isolated.
    render: (g: Graphics, attrs: PlayerSkinRenderAttributes) => void;
}

// Registration-ordered so getActivePlayerSkin resolves deterministically when
// more than one bit happens to be set.
const SKIN_REGISTRY: PlayerSkin[] = [];

export function registerPlayerSkin(skin: PlayerSkin): void {
    const existing = SKIN_REGISTRY.findIndex(s => s.flag === skin.flag);
    if (existing >= 0) SKIN_REGISTRY[existing] = skin;
    else SKIN_REGISTRY.push(skin);
}
...
export function getActivePlayerSkin(renderFlags: number | undefined): PlayerSkin | undefined {
    if (!renderFlags) return undefined;
    for (const skin of SKIN_REGISTRY) {
        if (renderFlags & skin.flag) return skin;
    }
    return undefined;
}

export function getAllPlayerSkins(): readonly PlayerSkin[] {
    return SKIN_REGISTRY;
}

export function getPlayerSkinByName(name: string): PlayerSkin | undefined {
...
const customSkinRegistry = new Map<string, CustomSkin>();

export function setCustomSkins(list: CustomSkin[] | undefined): void {
    customSkinRegistry.clear();
    if (Array.isArray(list)) {
        for (const s of list) if (s && s.id) customSkinRegistry.set(s.id, s);
    }
}

export function upsertCustomSkin(skin: CustomSkin): void {
    if (skin && skin.id) customSkinRegistry.set(skin.id, skin);
}
...
export function getCustomSkin(id: string | undefined): CustomSkin | undefined {
    return id ? customSkinRegistry.get(id) : undefined;
}

export function getAllCustomSkins(): CustomSkin[] {
 ...
 */
export function renderCustomSkinShapes(
    ...
    for (let i = 0; i < shapes.length; i++) {
        ...
        switch (s.t) {
            ...
            case 'polygon': {
                ...
                if (p.length >= 6) {
                    ctx.moveTo(p[0], p[1]);
                    for (let j = 2; j + 1 < p.length; j += 2) ctx.lineTo(p[j], p[j + 1]);
                    ctx.closePath();
                }
                break;
            }
...
registerPlayerSkin({
    flag: PlayerRenderFlags.Pumpkin,
    name: 'Pumpkin',
    render(g, attrs) {
        const ctx = g.ctx;
...
registerPlayerSkin({
    ...
    render(g, attrs) {
        ...
        for (const [bx, by] of [[-17, -14], [17, -14], [-17, 15], [17, 15]] as const) {
            ctx.beginPath();
