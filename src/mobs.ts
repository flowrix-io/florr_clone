 ...
 */
export function getEnemySizeScale(isPet: boolean | undefined, tier: string, mobType: string): number {
    if (isPet) return PET_SIZE_SCALING[tier] ?? 1;
    if (mobType === 'target_dummy') return DUMMY_SIZE_SCALING[tier] ?? 1;
    return 1;
}
