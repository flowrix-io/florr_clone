...
let host: EntityHost | undefined;

/** Install the ECS host. Called once, from the composition root. */
export function bindEntityHost(installed: EntityHost): void {
    host = installed;
}

export function requireEntityHost(): EntityHost {
    if (!host) {
        throw new Error(
            'entityRegistry: no ECS host installed. server.ts must call '
 ...
 */
const pendingRetirements: Entity[] = [];
const pendingRetirementSet = new Set<Entity>();
...
export function entityRetirementVersion(): number {
    return retirementVersion;
}

/** Queue an entity for destruction at the end of the current tick. */
export function retire(entity: Entity): void {
    if (pendingRetirementSet.has(entity)) return;
 ...
 */
export function retireNow(entity: Entity): boolean {
    pendingRetirementSet.add(entity);
    retirementVersion++;
...
export function isPendingRetirement(entity: Entity): boolean {
 ...
 */
export function isLiveForWire(world: World, entity: Entity): boolean {
 ...
 */
export function drainRetired(world: World): void {
...
export function pendingRetirementCount(): number {
...
export function cachedQuery(
    slot: { query?: Query; world?: World },
    world: World,
    all: ReadonlyArray<Parameters<World['query']>[0][number]>,
    none: ReadonlyArray<Parameters<World['query']>[0][number]> = [],
): Query {
