    ...
    socket.on('updateLoadout', (data: { loadout: (Item | null)[]; inventory: PlayerInventory; inPvpArena?: boolean; context?: 'regular' | 'maze' }) => {
        ...
        if (player) {
                ...
                if (petal && petal.type === 'petal') {
                    ...
                    if (isNewPetal && petal.petalType) {
                        ...
                        if (petalStats) {
                            ...
                            setTimeout(() => {
                                const restored = restorePetalSlot(targetId, index, petal);
                                if (!restored) return;
                                {
            ...
            if (splitState) {
                ...
                const otherHalf = players[otherHalfId];
                if (otherHalf) {
