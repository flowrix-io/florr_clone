...
export const REGISTER_DAILY_PER_ADDRESS = 10;
...
interface Bucket {
    tokens: number;
    /** When the bucket was last refilled, in ms — also its idleness for the sweep. */
    updatedAt: number;
    burst: number;
    refillPerSec: number;
}
...
export interface LimitVerdict {
    allowed: boolean;
    /** Seconds until one more attempt would be granted. Sent as Retry-After. */
    retryAfterSeconds: number;
    /** Player-facing text. Deliberately vague about which limit tripped. */
    message: string;
    /** Which limit refused, for logs. */
    scope: 'ip' | 'daily' | 'global' | null;
}

const ALLOWED: LimitVerdict = { allowed: true, retryAfterSeconds: 0, message: '', scope: null };
...
const globalRegisterBucket: Bucket = {
...
function waitSeconds(bucket: Bucket): number {
 ...
 */
function sweep(table: Map<string, Bucket>, now: number): void {
    if (table.size <= MAX_TRACKED_ADDRESSES) return;
    for (const [key, bucket] of table) {
        refill(bucket, now);
        if (bucket.tokens >= bucket.burst) table.delete(key);
    }
    ...
    if (table.size > MAX_TRACKED_ADDRESSES) {
        ...
        for (let i = 0; i < oldestFirst.length / 2; i++) table.delete(oldestFirst[i][0]);
    }
}

function bucketFor(table: Map<string, Bucket>, key: string, burst: number, refillPerSec: number, now: number): Bucket {
...
function denied(scope: 'ip' | 'daily' | 'global', retryAfterSeconds: number, message: string): LimitVerdict {
...
export function spendRegistration(
    ...
    if (ipBucket.tokens < 1) {
        return denied('ip', waitSeconds(ipBucket),
            'Too many accounts created from here. Please wait a few minutes.');
    }
    if (globalRegisterBucket.tokens < 1) {
        return denied('global', waitSeconds(globalRegisterBucket),
            'The server is creating accounts as fast as it will right now. Please try again shortly.');
    }
    if (accountsToday() >= REGISTER_DAILY_PER_ADDRESS) {
        return denied('daily', 3600,
 ...
 */
export function spendLoginAttempt(key: string, now: number = Date.now()): LimitVerdict {
...
export function refundLoginAttempt(key: string): void {
    const bucket = loginBuckets.get(key);
    if (bucket) bucket.tokens = Math.min(bucket.burst, bucket.tokens + 1);
}
 ...
 */
export function refusalLogLine(address: string, scope: string, now: number = Date.now()): string | null {
    suppressedRefusals++;
    if (now - lastRefusalLogAt < REFUSAL_LOG_INTERVAL_MS) return null;
...
export function limiterSnapshot(): { addresses: number; globalTokens: number } {
...
export function resetAccountLimiter(now: number = Date.now()): void {
