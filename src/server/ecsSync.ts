...
const liveEnemyShells = new Set<Enemy>();
 ...
 */
function enemySyncQuery(world: World): Query {
...
function maintainEnemyEntities(world: World, enemies: Enemy[], now: number): void {
    drainRemovedEnemies(world);

    if (++ticksSinceEnemyAudit < ENEMY_AUDIT_INTERVAL_TICKS) return;
    ...
    if (auditEnemyEntities(world, enemySyncQuery(world)) === 0) return;
        ...
        for (let i = 0; i < chunk.count; i++) {
            ...
            if (enemy.knockbackX || enemy.knockbackY) {
                if (knock !== undefined) {
                    ...
                    pendingKnockback.push(entities[i] as Entity);
            ...
            if (isDead && !hasDead) pendingDead.push(entities[i] as Entity);
            ...
            if (enemy.targetPlayerId !== undefined && ai !== undefined) {
                ...
                if (target !== undefined) ai.targetPlayer[i] = target;
...
export function syncFromEcs(world: World, enemies: Enemy[]): void {
    ...
    enemySyncQuery(world).chunks(chunk => {
        ...
        for (let i = 0; i < chunk.count; i++) {
            ...
            // consumer.
            if (ai !== undefined) {
                const target = ai.targetPlayer[i] as Entity;
            ...
            if (seg !== undefined) {
                const leader = seg.leader[i] as Entity;
                const head = seg.head[i] as Entity;
