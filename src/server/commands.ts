...
import { ServerPlayer, PlayerRenderFlags } from '../player';
...
import { ENEMY_COUNT, setPlayerCorrupted } from './gameState';
...
import { setTargetBotCount, getTargetBotCount, MAX_BOT_COUNT, isBot } from './botManager';
...
export interface CommandHandlerDependencies {
    ...
    spawnMob: (mobType: string, rarity: string, x?: number, y?: number, count?: number, stack?: boolean) => void;
...
): { id: string; player: ServerPlayer; socket: AuthenticatedSocket | undefined } | null {
    const direct = players[playerIdentifier];
    if (direct) {
        return {
            id: playerIdentifier,
            player: direct,
            socket: io.sockets.sockets.get(playerIdentifier) as AuthenticatedSocket,
        };
    }
    ...
    if (trimmedCommand.startsWith('save')) {
        ...
        if (arg !== undefined && arg.toLowerCase() === 'default') {
            ...
            if (isNaN(requested) || requested < 0) {
                ...
                const applied = Math.min(requested, MAX_BOT_COUNT);
                setTargetBotCount(applied);
                sendOutput(
                    applied === requested
                        ? `Bot count target set to ${applied}.`
                        : `Bot count target set to ${applied} (requested ${requested}, capped at ${MAX_BOT_COUNT}).`,
                    socketId, io
                );
            }
        }
        ...
        const isStackWord = (s: string | undefined): boolean =>
            s !== undefined && ['stack', 'stacked'].includes(s.toLowerCase());
        const isUnstackWord = (s: string | undefined): boolean =>
            s !== undefined && ['unstack', 'unstacked', 'nostack'].includes(s.toLowerCase());
        const isStackFlag = (s: string | undefined): boolean => isStackWord(s) || isUnstackWord(s);
        ...
        if (parts.length < 3) {
            ...
            let amountTok: string | undefined;
            let stackTok: string | undefined;
            ...
            if (stackTok !== undefined) {
                if (isStackWord(stackTok)) stack = true;
                else if (isUnstackWord(stackTok)) stack = false;
                else {
            ...
            if (!hasCoords && socketId && players[socketId]) {
        ...
        // teleport_all <x> <y> — move every player, bots included, to one spot.
        const parts = trimmedCommand.split(' ');
        if (parts.length !== 3) {
            sendOutput('Usage: teleport_all <x> <y>', socketId, io);
            sendOutput('  Teleports every online player and bot to (x, y).', socketId, io);
        ...
        for (const [pid, player] of Object.entries(players)) {
            ...
            if (isBot(pid)) {
                // No socket behind a bot — observers snap it into place anyway
                // via the client's TELEPORT_SNAP_DISTANCE cut on large jumps.
                movedBots++;
                continue;
            }

            // A splitter half owns no socket — address the client that drives it.
            io.to(getOriginalSocketId(pid)).emit('playerTeleported', {
                newX: x,
                newY: y,
                playerId: pid
            });
        ...
        const skinNames = Object.keys(PlayerRenderFlags).filter(k => isNaN(Number(k)));
        const parts = trimmedCommand.split(' ');
        if (parts.length !== 3) {
        ...
        let renderFlags: number | null;
        if (skinArg.toLowerCase() === 'none') {
            renderFlags = 0;
        } else {
            const matchedKey = skinNames.find(k => k.toLowerCase() === skinArg.toLowerCase());
            if (matchedKey) {
                renderFlags = PlayerRenderFlags[matchedKey as keyof typeof PlayerRenderFlags];
            } else {
        ...
        if (targetPlayer) {
            ...
            if (targetSocket?.userId) savePlayerProgress(targetPlayer, targetSocket.userId);
        ...
        const split = splitPlayers.get(getOriginalSocketId(targetPlayer.id));
        const halves = split ? [split.player1, split.player2] : [targetPlayer];
        for (const half of halves) {
            if (players[half.id]) setPlayerCorrupted(players[half.id], corrupted);
        }

        sendOutput(
            `${corrupted ? 'Corrupted' : 'Cleansed'} ${targetPlayer.name} (${targetPlayer.id})`
        ...
        if (parts.length === 4 || parts.length === 5) {
            ...
            if (targetPlayer && targetPlayerId) {
                ...
                addItem(targetPlayer.inventory, rarity, itemKey, amount);
                ...
                const offlineUserId = database.getUserIdByUsername(playerIdentifier);
                if (offlineUserId) {
                    const progress = database.getPlayerByUserId(offlineUserId) || { totalXP: 0 };
                    if (!progress.inventory) progress.inventory = {};
                    if (!progress.inventory[rarity]) progress.inventory[rarity] = {};
                    progress.inventory[rarity][itemKey] = (progress.inventory[rarity][itemKey] || 0) + amount;
                    database.savePlayer(offlineUserId, progress);
        ...
        if (arg === 'status') {
            const info = getSimulatedTickSpikeInfo();
            if (!info) sendOutput('No simulated tick spike active.', socketId, io);
            else sendOutput(`Simulating dt=${info.deltaSeconds}s for ${(info.remainingMs / 1000).toFixed(1)}s more.`, socketId, io);
        } else if (arg === 'cancel' || arg === 'stop') {
            const wasActive = cancelSimulatedTickSpike();
            ...
            if (!Number.isFinite(deltaSeconds) || !Number.isFinite(durationSeconds)) {
                ...
                const result = simulateTickSpike(deltaSeconds, durationSeconds * 1000);
