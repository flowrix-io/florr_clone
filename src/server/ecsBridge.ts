...
import { spawnPlayer } from '../ecs/prefabs';
 ...
 */
export function aiTypeOf(aiType: string | undefined): C.AiType {
    switch (aiType) {
...
export function attachMobBehaviour(
    ...
    if ((world.get(entity, C.Speed, 'current') as number) > 0) {
        world.add(entity, C.PassiveMotion, { state: C.PassiveState.Idle, stateStart: now });
    ...
    const config = stats ?? getMobStats(enemy.type, enemy.tier);
    ...
    if (config?.spawn_waves && config.spawn_waves.length > 0) {
        world.add(entity, C.SpawnWaveState, {
            previousHealth: world.get(entity, C.Health, 'current') as number,
        });
    }
}
