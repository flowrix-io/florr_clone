...
interface CertFacts {
    /** Inside its validity window, with a day of margin at the end. */
    usable: boolean;
    /** Names nothing but localhost / loopback addresses. */
    localOnly: boolean;
}

/** Is every name on this certificate a loopback name? */
function namesAreLocalOnly(cert: crypto.X509Certificate): boolean {
...
function inspect(certPath: string): CertFacts | null {
 ...
 */
export function generateDevCert(paths: CertPaths): boolean {
    ...
    try {
        ...
        try { fs.chmodSync(paths.keyPath, 0o600); } catch { /* best effort */ }
        return true;
    } catch (e: any) {
 ...
 */
export function resolveTlsPaths(primary: CertPaths, fallback: CertPaths): CertPaths | null {
    const primaryFacts = inspect(primary.certPath);
    const primaryKeyExists = fs.existsSync(primary.keyPath);
    ...
    if (primaryFacts && !primaryFacts.localOnly) {
        console.error(
            `[CERT] ${path.basename(primary.certPath)} is expired or not yet valid, and it is not a `
    ...
    if (primaryFacts) {
        console.warn(`[CERT] ${path.basename(primary.certPath)} has expired; using a generated localhost certificate instead.`);
    } else if (primaryKeyExists) {
        console.warn(`[CERT] ${path.basename(primary.certPath)} is missing or unreadable; generating a localhost certificate.`);
    }

    const fallbackFacts = inspect(fallback.certPath);
    if (fallbackFacts?.usable && fs.existsSync(fallback.keyPath)) {
        return fallback;
    }
    ...
    console.log(
        `[CERT] Generated ${path.basename(fallback.certPath)} for localhost `
