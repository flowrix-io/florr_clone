...
// actually moves on.
const BOT_COUNT_JITTER_MIN_INTERVAL_MS = 25_000;
 ...
 */
let spawnAreaCache: MapElement[] | null = null;
function getSpawnAnchorElements(): MapElement[] {
    if (spawnAreaCache) return spawnAreaCache;
    const commonZones = WORLD_MAP.filter(
        e => e.type === 'spawn' && e.width > 0 && e.height > 0
            && e.properties?.spawnType === 'common'
    );
    const safeBiomes = WORLD_MAP.filter(
        e => e.type === 'biome' && e.width > 0 && e.height > 0
...
function pickBotSpawnPosition(): { x: number; y: number } {
    ...
    if (anchors.length > 0) {
        ...
        for (const anchor of shuffled) {
            ...
            if (baseArea.width <= 0 || baseArea.height <= 0) continue;
        ...
        for (const anchor of shuffled) {
            for (let i = 0; i < 12; i++) {
                const x = (anchor.x + Math.random() * anchor.width) * SCALE_FACTOR;
                const y = (anchor.y + Math.random() * anchor.height) * SCALE_FACTOR;
                if (!population().isPositionInsideWall(x, y, PLAYER_SIZE)) return { x, y };
            }
        }
    ...
    const safe = population().findSafeSpawnPosition(
        { x: 0, y: 0, width: ACTUAL_WORLD_WIDTH / SCALE_FACTOR, height: ACTUAL_WORLD_HEIGHT / SCALE_FACTOR },
        50
    );
    if (safe) return safe;
 ...
 */
function cullScore(id: string): number {
    const bot = players[id];
    if (!bot) return Number.MAX_SAFE_INTEGER;
    if (bot.isDead) return Number.MAX_SAFE_INTEGER;
    const nearest = nearestRealPlayer(bot.x, bot.y);
    if (!nearest) return 0;   // nobody watching anyone — order doesn't matter
...
const _avoidQueryScratch: (Enemy)[] = [];
...
function avoidNearbyMobs(
    ...
    out.x = 0;
    out.y = 0;
    const near = queryEnemiesNear(bot.x, bot.y, MOB_AVOID_QUERY_RADIUS, _avoidQueryScratch);
    for (let i = 0; i < near.length; i++) {
        const enemy = near[i];
        ...
        if (enemy.ownerId === bot.id) continue;
        const dx = bot.x - mobX(enemy.entity);
        const dy = bot.y - mobY(enemy.entity);
        const d2 = dx * dx + dy * dy;
        if (d2 === 0) continue;
        const ring = PLAYER_SIZE / 2 + getMobRadius(enemy) + MOB_AVOID_MARGIN;
...
function followPath(
    ...
    speedMult: number,
    petalExt: number,
    avoidExceptId: string | null = null
): boolean {
...
function resetOscillationSampler(bot: ServerPlayer, state: BotAIState, now: number): void {
    ...
    if (state.oscTrackX) state.oscTrackX.length = 0;
    if (state.oscTrackY) state.oscTrackY.length = 0;
}
...
function detectOscillation(bot: ServerPlayer, state: BotAIState, now: number): boolean {
    ...
    if (tx.length === OSC_NET_WINDOW) {
        ...
        const drift = Math.sqrt(
            (tx[tx.length - 1] - tx[0]) ** 2 + (ty[ty.length - 1] - ty[0]) ** 2
        );
...
function pickUnstickDirection(bot: ServerPlayer, state: BotAIState): { x: number; y: number } {
    ...
    const cx = ACTUAL_WORLD_WIDTH / 2 - bot.x;
    const cy = ACTUAL_WORLD_HEIGHT / 2 - bot.y;
    const cd = Math.sqrt(cx * cx + cy * cy);
    if (cd > 1) return { x: cx / cd, y: cy / cd };
    ...
    for (let botIndex = 0; botIndex < botRoster.length; botIndex++) {
        ...
        if (target) {
            ...
            if (d > standoff + 80) {
                ...
                if (mode.kind !== 'normal' && followPath(bot, state, now, mobX(target.enemy.entity), mobY(target.enemy.entity), 0.95, extendedPetalExt, target.enemy.id)) {
                    continue;
                }
