...
import {
    selectLootRecipients, lootSlotsForTier, BASE_LOOT_SLOTS, payFullXpToEach, withoutBots,
} from './lootEligibility';

export function runLootEligibilitySelfTest(): string[] {
    ...
    const solo = (contributors: Map<string, number>, tier: string) =>
        selectLootRecipients(
            contributors,
            tier,
            Array.from(contributors.entries()).sort((a, b) => b[1] - a[1]).map(e => e[0]),
            id => [id],
        );

    // -- per-tier caps --------------------------------------------------------
    const crowd = new Map<string, number>();
    for (let i = 0; i < 40; i++) crowd.set(`p${i}`, 1000 - i);

    for (const tier of ['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic']) {
        check(`${tier} caps at ${BASE_LOOT_SLOTS}`, solo(crowd, tier).length === BASE_LOOT_SLOTS,
            `got ${solo(crowd, tier).length}`);
    }
    for (const [tier, want] of [['ultra', 15], ['super', 20], ['unique', 25]] as const) {
    ...
    check('apex inherits unique', lootSlotsForTier('apex') === lootSlotsForTier('unique'));
    check('unknown tier falls back to base', lootSlotsForTier('not_a_tier') === BASE_LOOT_SLOTS);

    // -- the cap goes to the biggest hitters ----------------------------------
    check('slots go to the top damage dealers',
        JSON.stringify(solo(crowd, 'common')) === JSON.stringify(['p0', 'p1', 'p2', 'p3']),
        JSON.stringify(solo(crowd, 'common')));

    // -- damage is required ---------------------------------------------------
    check('a zero-damage contributor is not eligible',
        !solo(new Map([['a', 100], ['b', 0], ['c', 50]]), 'common').includes('b'));
    check('no contributors means nobody', solo(new Map(), 'unique').length === 0);
    check('fewer contributors than slots is fine',
        solo(new Map([['a', 5], ['b', 3]]), 'unique').length === 2);
    ...
    // -- squads must not blow past the cap ------------------------------------
    {
        ...
        const expand = (id: string) => members[id] ?? [id];

        const common = selectLootRecipients(contributors, 'common', ranked, expand);
        check('4 squads of 10 still yield only 4 looters on a common mob',
            common.length === 4, `got ${common.length}`);
        check('an idle squad member never loots', !common.includes('idler'));

        const ultra = selectLootRecipients(contributors, 'ultra', ranked, expand);
    ...
    {
        const pay = (recipients: string[], accounts: Record<string, string> = {}, mult: Record<string, number> = {}) => {
            const out: Array<{ id: string; xp: number }> = [];
            payFullXpToEach(recipients, 30,
                id => accounts[id],
                id => mult[id] ?? 1,
                (id, xp) => out.push({ id, xp }));
            return out;
        };

        const four = pay(['a', 'b', 'c', 'd']);
        check('every looter is paid', four.length === 4, JSON.stringify(four));
        check('each gets the FULL amount, not a split share',
            four.every(p => p.xp === 30), JSON.stringify(four.map(p => p.xp)));

        // A split flower is two player records for one person.
        const split = pay(['7', '7_split2', 'z'], { '7': 'acc7', '7_split2': 'acc7', z: 'accZ' });
        check('a split player is paid once, not twice',
            split.filter(p => p.id === '7' || p.id === '7_split2').length === 1, JSON.stringify(split));
        check('and the other player is still paid', split.some(p => p.id === 'z'));

        // Bots have no account and must stay distinct from one another.
        check('recipients without an account are not collapsed together',
            pay(['bot_a', 'bot_b']).length === 2);
        ...
        check('multiplier applies per recipient',
            paidA?.xp === 15 && paidB?.xp === 30, JSON.stringify(scaled));
    ...
    // -- bots must not take slots from real players ---------------------------
    {
        ...
        const filtered = withoutBots(busy);
        ...
        for (let i = 0; i < 8; i++) mixed.set('bot_' + i, 1000 - i);
        for (let i = 0; i < 6; i++) mixed.set('h' + i, 100 - i);
        const m = solo(withoutBots(mixed), 'common');
        check('the cap applies among humans', m.length === BASE_LOOT_SLOTS, JSON.stringify(m));
        check('and it is the top humans by damage',
            JSON.stringify(m) === JSON.stringify(['h0', 'h1', 'h2', 'h3']), JSON.stringify(m));

        // A mob only bots touched rewards nobody, so it drops nothing.
        check('a bot-only kill has no recipients',
            withoutBots(new Map([['bot_a', 100], ['bot_b', 50]])).size === 0);
    }
