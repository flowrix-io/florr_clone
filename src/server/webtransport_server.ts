...
import type { WSServer, ServerTransport } from '../ws_server';
import { MAX_INBOUND_MESSAGE_BYTES } from '../ws_server';
import { FrameReader, frameMessage } from '../net/stream_framing';
import { SessionAdmission } from './webtransport_limits';
...
export interface WebTransportServerOptions {
    ...
    port: number;
    /** Path clients open the session against. */
    path?: string;
    certPath: string;
    keyPath: string;
    /**
     * Bind address. Defaults to trying `::` (dual-stack, matching what the uWS
     * TCP listener does) and falling back to `0.0.0.0` where IPv6 is off — an
     * IPv4-only QUIC socket would silently miss every client that resolved the
     * host to an IPv6 address, which on a dev box `localhost` usually does.
     */
    host?: string;
}

/** What server.ts publishes at `GET /transport-info` once the listener is up. */
export interface WebTransportAdvertisement {
    webtransport: true;
    port: number;
    path: string;
    /** Present only for a certificate no public CA vouches for — see below. */
    certHashes?: string[];
}
...
const liveSessions = new Set<WebTransportSessionTransport>();
...
export function getWebTransportStats(): { sessions: number; maxSessions: number } {
...
let advertisement: WebTransportAdvertisement | null = null;

/** Null until (and unless) a WebTransport listener is actually accepting sessions. */
export function getWebTransportAdvertisement(): WebTransportAdvertisement | null {
...
function pinnableCertificateHashes(pem: string): string[] {
    const block = pem.match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/);
    if (!block) return [];

    let cert: crypto.X509Certificate;
    try {
        cert = new crypto.X509Certificate(block[0]);
    } catch {
        return [];
    }

    const key: any = cert.publicKey;
    if (key.asymmetricKeyType !== 'ec') return [];
    if (key.asymmetricKeyDetails?.namedCurve !== 'prime256v1') return [];

    const from = Date.parse(cert.validFrom);
    const to = Date.parse(cert.validTo);
    if (!Number.isFinite(from) || !Number.isFinite(to)) return [];
    if (to - from > MAX_PINNABLE_VALIDITY_MS) return [];
...
class WebTransportSessionTransport implements ServerTransport {
    ...
    send(payload: Uint8Array): number {
        ...
        if (!writer || this.closed || this.closing) return -1;
        if (this.buffered > MAX_BUFFERED_BYTES) return 2;

        const framed = frameMessage(payload);
        this.buffered += framed.byteLength;
        this.lastWrite = writer.write(framed).then(
            () => { this.buffered -= framed.byteLength; },
            () => { this.buffered -= framed.byteLength; this.fireClose(); },
        );
    ...
    close(graceful: boolean): void {
        ...
        const deadline = new Promise<void>(resolve => setTimeout(resolve, CLOSE_DRAIN_MS));
 ...
 */
function peerIp(peerAddress: unknown): string {
    if (typeof peerAddress !== 'string' || peerAddress === '') return '';
...
async function readFirstStream(session: any): Promise<{ stream: any; reader: any }> {
    const reader = session.incomingBidirectionalStreams.getReader();
    const timeout = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('client never opened a stream')), STREAM_WAIT_MS);
    });
    const { value, done } = await Promise.race([reader.read(), timeout]) as any;
    if (done || !value) throw new Error('session closed before opening a stream');
 ...
 * than merely wrong.
 */
function drainExtraStreams(session: any, reader: any, transport: WebTransportSessionTransport): void {
    void (async () => {
        let extras = 0;
        try {
            for (;;) {
                ...
                if (done) break;
                extras++;
                try { value.readable?.cancel(); } catch { /* already gone */ }
                try { value.writable?.abort(); } catch { /* already gone */ }
                if (extras > MAX_EXTRA_STREAMS) {
                    console.warn(
                        `[WT] ${transport.remoteAddress || 'unknown ip'} opened ${extras} extra streams — closing session`,
                    );
                    transport.close(false);
                    return;
                }
    ...
    void (async () => {
        try {
            ...
            for (;;) {
                ...
                if (done) break;
                try { value.cancel(); } catch { /* already gone */ }
            }
        } catch {
...
async function handleSession(io: WSServer, session: any): Promise<void> {
    await session.ready;
    ...
    let transport: WebTransportSessionTransport | null = null;
    try {
        ...
        const frames = new FrameReader(MAX_INBOUND_MESSAGE_BYTES);
        for (;;) {
 ...
 */
export async function startWebTransportServer(
    io: WSServer,
    options: WebTransportServerOptions,
): Promise<WebTransportAdvertisement | null> {
    ...
    try {
        ...
        privKey = fs.readFileSync(options.keyPath, 'utf8');
    } catch (e) {
    ...
    for (const host of hosts) {
        ...
        try {
            candidate = new Http3Server({
                ...
                initialStreamFlowControlWindow: MAX_INBOUND_MESSAGE_BYTES,
                streamFlowControlWindowSizeLimit: 4 * MAX_INBOUND_MESSAGE_BYTES,
                initialSessionFlowControlWindow: 2 * MAX_INBOUND_MESSAGE_BYTES,
                sessionFlowControlWindowSizeLimit: 8 * MAX_INBOUND_MESSAGE_BYTES,
            });
            candidate.startServer();
            await Promise.race([
                candidate.ready,
                new Promise((_, reject) => setTimeout(
                    () => reject(new Error(`listener not ready after ${LISTEN_TIMEOUT_MS}ms`)),
                    LISTEN_TIMEOUT_MS,
                )),
            ]);
            server = candidate;
            ...
            lastError = e;
            try { candidate?.stopServer(); } catch { /* never started */ }
        }
    }
    if (!server) {
        ...
        try {
            ...
            for (;;) {
                ...
                handleSession(io, value).catch(e => {
                    ...
                    try { value.close(); } catch { /* already gone */ }
                });
            }
        } catch (e) {
    ...
    console.log(
        `[WT] WebTransport listening on udp/${options.port}${path}`
        + (certHashes.length > 0
            ? ' (advertising certificate hashes)'
            : ' (relying on CA validation of the server certificate)'),
    );
    console.log(
        `[WT] Limits: ${MAX_SESSIONS} sessions, ${MAX_SESSIONS_PER_ADDRESS}/address, `
        + `${MAX_INBOUND_MESSAGE_BYTES / 1024}KB max message, ${IDLE_TIMEOUT_MS / 1000}s idle timeout`,
    );
    return advertisement;
}
