...
class SVGRendererWrapper {
    ...
    private applyPathAnimations(svgString: string, time: number): string {
        ...
        return svgString.replace(pathWithAnimateRegex, (match, pathAttrs: string, animateAttrs: string) => {
            // Only handle d attribute animations
            const attrNameMatch = animateAttrs.match(/attributeName="([^"]*)"/);
            if (!attrNameMatch || attrNameMatch[1] !== 'd') return match;

            const durMatch = animateAttrs.match(/dur="([^"]*)"/);
            const valuesMatch = animateAttrs.match(/values="([^"]*)"/);
            if (!durMatch || !valuesMatch) return match;
            ...
            if (keyframes.length < 2) return match;
            ...
            if (idx >= keyframes.length - 1) idx = keyframes.length - 2;
            ...
            if (nums1.length !== nums2.length || nums1.length === 0) return match;
